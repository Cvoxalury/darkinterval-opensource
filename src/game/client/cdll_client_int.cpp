...
public:
	...
	{
		return "blank"; // TODO: can't we just remove this entirely? It has ZERO business being in SP SDK.
	}
};
...
/*
	ConColorMsg(g_ColorYellow, "=====================================\n");
	ConColorMsg(g_ColorYellow, "\n");
	ConColorMsg(g_ColorYellow, "\n");
	ConColorMsg(g_ColorYellow, "===========WELCOME, CITIZEN===========\n");
	ConColorMsg(g_ColorYellow, "\n");
	ConColorMsg(g_ColorYellow, "\n");
	ConColorMsg(g_ColorYellow, "=====================================\n");
*/
	CommandLine()->AppendParm("-noborder", NULL);

	// DI NEW: Make sure HDR is on
	ConVarRef hdrcheck("mat_hdr_level");
	if (hdrcheck.GetInt() != 2) hdrcheck.SetValue(2);
	Msg("Using HDR shader pipeline (mat_hdr_level is %i)\n", hdrcheck.GetInt());
#endif
	return true;
}
...
void CHLClient::HudUpdate( bool bActive )
{
	...
	hudlcd->SetGlobalStat( "(time_int)", VarArgs( "%d", (int)CURTIME ) );
	hudlcd->SetGlobalStat( "(time_float)", VarArgs( "%.2f", CURTIME ) );
...
void CHLClient::LevelInitPreEntity( char const* pMapName )
{
	...
	tempents->LevelInit();
#ifndef DARKINTERVAL
	ResetToneMapping(1.0); // DI REMOVE - it appears this is responsible for level changes briefly darkening the tonemapping, annoying
#endif
	...
	ConVarRef hdrcheck("mat_hdr_level");
//	if (hdrcheck.GetInt() != 2) hdrcheck.SetValue(2);
	if( hdrcheck.GetInt() < 2)
		Msg("Using LDR shader pipeline (mat_hdr_level is %i)\n", hdrcheck.GetInt());
	else
		Msg("Using HDR shader pipeline (mat_hdr_level is %i)\n", hdrcheck.GetInt());
#endif
}
