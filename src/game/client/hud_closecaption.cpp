...
#include "cbase.h"
#ifdef DARKINTERVAL
#define USE_2004_CC_CODE 1 // 2004 closed captions didn't need to get compiled into .dat, work as plain txt, which is a no-brainer to have.
#endif
#if USE_2004_CC_CODE
...
ConVar closecaption("closecaption", "1", FCVAR_ARCHIVE | FCVAR_USERINFO, "Enable close captioning.");
...
static ConVar cc_captiontrace("cc_captiontrace", "0", 0, "Show missing closecaptions (0 = no, 1 = devconsole, 2 = show in hud)");
...
void CHudCloseCaption::Paint(void)
{
	...
	if (totalheight != m_nGoalHeight)
	{
		...
		m_flGoalHeightStartTime = CURTIME;
		m_flGoalHeightFinishTime = CURTIME + m_flGrowTime;
	}
		...
		m_flGoalHeightFinishTime > m_flGoalHeightStartTime)
	{
		...
		float frac = (CURTIME - m_flGoalHeightStartTime) / dt;
...
void CHudCloseCaption::ProcessSentenceCaptionStream(char const *tokenstream)
{
	...
	if (Q_strlen(tokenname) > 0)
	{
		...
		CaptionRepeat entry;
		entry.m_nTokenIndex = g_pVGuiLocalize->FindIndex(tokenname);

		int idx = m_CloseCaptionRepeats.Find(entry);
		if (m_CloseCaptionRepeats.InvalidIndex() == idx)
		{
			entry.m_flLastEmitTime = CURTIME;
		...
		else
		{
			...
			if (CURTIME < (entry.m_flLastEmitTime + entry.m_flInterval))
			{
				return;
			}
	...
	while (p && Q_strlen(tokenname) > 0)
	{
		...
		{
			g_pVGuiLocalize->ConvertANSIToUnicode(VarArgs("!!!%s", tokenname), caption, sizeof(caption));
		}
	...
	/*
	char foo[ 2048 ];
	g_pVGuiLocalize->ConvertUnicodeToANSI( caption_full, foo, sizeof( foo ) );
	Msg( "'%s'\n", foo );
	*/
...
void CHudCloseCaption::ProcessCaption(char const *tokenname, float duration, bool fromplayer /* = false */)
{
	...
	if (!LookupUnicodeText(tokenname, caption, sizeof(caption) / sizeof(wchar_t)))
	{
		...
		g_pVGuiLocalize->ConvertANSIToUnicode(VarArgs("--> Missing Caption[%s]", tokenname), caption, sizeof(caption));
	}
	else
	{
...
void CHudCloseCaption::MsgFunc_CloseCaption(bf_read &msg)
{
	...
	bool warnonmissing = false; //flagbyte & CLOSE_CAPTION_WARNIFMISSING ? true : false;
	...
	if (bIsMale || bIsFemale)
	{
		...
		wchar_t *pcheck = g_pVGuiLocalize->Find(szTestName);
		if (pcheck)
		{
...
void OnCaptionLanguageChanged(IConVar *pConVar, const char *pOldString, float flOldValue)
{
	...
	else
	{
		...
		{
			char dbfile[512];
			Q_snprintf(dbfile, sizeof(dbfile), "resource/closecaption_%s.txt", uilanguage);
			g_pVGuiLocalize->AddFile(dbfile, "GAME", true);
		}
	}
	DevMsg("cc_lang = %s\n", var.GetString());
}
		...
		if ( height > avail_height )
		{
			...
			if ( iLinesToMove )
			{
 				...
 				if ( flTimePostMove < CAPTION_PAN_FADE_TIME )
				{
					...
					{
						wu->SetFadeStart( CURTIME );
					}

					// Fade out quickly
					float flFadeTime = (CURTIME - wu->GetFadeStart()) /  CAPTION_PAN_FADE_TIME;
...
//			DevMsg("Performing CaptionLookup for index %i, dirintex %i\n", nFileIndex, caption->dirindex);
