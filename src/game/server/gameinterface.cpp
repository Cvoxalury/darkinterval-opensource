...
#endif
#ifdef DARKINTERVAL
#include "world.h" // for retrieving map data such as map title
#endif
...
ConVar sv_force_transmit_ents( "sv_force_transmit_ents", "0", FCVAR_DEVELOPMENTONLY, "Will transmit all entities to client, regardless of PVS conditions (will still skip based on transmit flags, however)." );
...
#ifdef DARKINTERVAL
#if 0
	// Make sure old shader files are deleted via filesystem
	const char *szJunkFileName = "shaders/fxc/refract_ps20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/refract_ps20b.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/refract_vs20.vcs";
	filesystem->RemoveFile(szJunkFileName);

	szJunkFileName = "shaders/fxc/aftershock_ps20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/aftershock_ps20b.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/aftershock_ps30.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/aftershock_vs20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/aftershock_vs30.vcs";
	filesystem->RemoveFile(szJunkFileName);

	szJunkFileName = "shaders/fxc/cloak_blended_pass_ps20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/cloak_blended_pass_ps20b.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/cloak_blended_pass_ps30.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/cloak_blended_pass_vs20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/cloak_blended_pass_vs30.vcs";
	filesystem->RemoveFile(szJunkFileName);

	szJunkFileName = "shaders/fxc/emissive_scroll_blended_pass_ps20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/emissive_scroll_blended_pass_ps20b.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/emissive_scroll_blended_pass_ps30.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/emissive_scroll_blended_pass_vs20.vcs";
	filesystem->RemoveFile(szJunkFileName);
	szJunkFileName = "shaders/fxc/emissive_scroll_blended_pass_vs30.vcs";
	filesystem->RemoveFile(szJunkFileName);
...
//#endif
	...
	if ( !mdlcache )
		return false;
#ifndef DARKINTERVAL // doesn't matter for DI
...
#ifndef DARKINTERVAL // DI doesn't care about cheats
...
#ifndef _XBOX
#ifndef DARKINTERVAL // doesn't matter for DI	// Add HLTV director 
...
#ifndef DARKINTERVAL // doesn't matter for DI	// init the gamestatsupload connection
...
#endif
#ifdef DARKINTERVAL	// DI delete junk file
	const char *szJunkFileName = "demoheader.tmp";
	filesystem->RemoveFile(szJunkFileName);
...
#ifndef DARKINTERVAL // doesn't matter for DI, all particles are precached
...
#ifndef DARKINTERVAL // doesn't matter for DI	// load MOTD from file into stringtable
	LoadMessageOfTheDay();
#endif
...
{
	if ( m_fAutoSaveDangerousTime != 0.0f && m_fAutoSaveDangerousTime < CURTIME )
	{
		// The safety timer for a dangerous auto save has expired
		CBasePlayer *pPlayer = UTIL_PlayerByIndex( 1 );

		if ( pPlayer && ( pPlayer->GetDeathTime() == 0.0f || pPlayer->GetDeathTime() > CURTIME )
...
#ifdef DARKINTERVAL
	{ "ch00_docks", "#darkinterval_SaveGame_Docks" },
	{ "ch00_dream", "#darkinterval_SaveGame_Prologue" },
	{ "ch00_ship", "#darkinterval_SaveGame_Prologue" },
	{ "ch01_industrial", "#darkinterval_SaveGame_Ind" },
	{ "ch01_terminal", "#darkinterval_SaveGame_Term" },
	{ "ch01_construction", "#darkinterval_SaveGame_Con" },
	{ "ch01_consulcast", "#darkinterval_SaveGame_Cast" },
	{ "ch02_arcadeexterior", "#darkinterval_SaveGame_ArcExt" },
	{ "ch02_arcadeinterior", "#darkinterval_SaveGame_ArcInt" },
	{ "ch02_radiostation", "#darkinterval_SaveGame_Radio" },
	{ "ch02_subt", "#darkinterval_SaveGame_Subt" },
	{ "ch03_lab", "#darkinterval_SaveGame_Lab" },
	{ "ch04_warehouses", "#darkinterval_SaveGame_Wares" },
	{ "ch04_workhouses", "#darkinterval_SaveGame_Works" },
	{ "ch04_refinery", "#darkinterval_SaveGame_Refinery" },
	{ "ch05_under", "#darkinterval_SaveGame_Under" },
	{ "ch05_Hydra", "#darkinterval_SaveGame_Hydra" },
	{ "ch06_Apcride", "#darkinterval_SaveGame_APCRide" },
	{ "ch06_Outerwall", "#darkinterval_SaveGame_Outerwall" },
	{ "ch07_Craters", "#darkinterval_SaveGame_Craters" },
	{ "ch07_Canyon", "#darkinterval_SaveGame_Canyon" },
	{ "ch07_Scrapland", "#darkinterval_SaveGame_Scrapland" },
	{ "ch08_Eli", "#darkinterval_SaveGame_Eli" },
	{ "ch08_Junkyard", "#darkinterval_SaveGame_Junkyard" },
	{ "ch09_Antcaves", "#darkinterval_SaveGame_Antcaves" },
	{ "ch09_Haven", "#darkinterval_SaveGame_Haven" },
	{ "ch09_Digger", "#darkinterval_SaveGame_Digger" },
	{ "ch09_Beach", "#darkinterval_SaveGame_Beach" },
	{ "ch10_Depot", "#darkinterval_SaveGame_Depot" },
	{ "test_", "#darkinterval_SaveGame_Test" },
#endif
#endif
};
...
void CServerGameDLL::GetSaveComment( char *text, int maxlength, float flMinutes, float flSeconds, bool bNoTime )
{
	...
	if (!pName)
	{
#ifndef DARKINTERVAL // todo - see about reenabling it for fanmade maps?
		if (GetWorldEntity()->GetMapTitle())
		{
			pName = GetWorldEntity()->GetMapTitle(); // mapname;
		}
		else
#endif
		pName = mapname;
	...
	Q_snprintf( text, maxlength, "%-64.64s %03d:%02d", pName, (minutes + minutesAdd), seconds );
		Msg("Saving the game, elapsed play time is: %03d:%02d, player health is: %.0f\n", 
			pName, (minutes + minutesAdd), seconds, (float)(player->GetHealth() / player->GetMaxHealth()));
#endif
#endif
	// add the elapsed time at the end of the comment, for the ui to parse out
		Q_snprintf(text, maxlength, "%-64.64s %03d:%02d", pName, (minutes + minutesAdd), seconds);
...
#ifdef DARKINTERVAL
		CHL2_Player* pPortalPlayer = dynamic_cast<CHL2_Player*>( pPlayer );
#else
