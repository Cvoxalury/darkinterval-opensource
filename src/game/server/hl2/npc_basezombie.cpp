...
#include "gib.h"
#ifdef DARKINTERVAL
#include "te_effect_dispatch.h" // for the particle fx on split zombies
#endif
...
#ifdef DARKINTERVAL // more convenient than scripted sequences or actbusy
int CNPC_BaseZombie::ACT_ZOMBIE_SLUMP_A;
int CNPC_BaseZombie::ACT_ZOMBIE_SLUMP_A_RISE;
int CNPC_BaseZombie::ACT_ZOMBIE_SLUMP_B;
int CNPC_BaseZombie::ACT_ZOMBIE_SLUMP_B_RISE;
#endif
...
ConVar zombie_ambushdist( "zombie_ambushdist", "16000" ); // this just did nothing!
#endif
//=========================================================
...
CNPC_BaseZombie::~CNPC_BaseZombie()
{
	g_numZombies--;
#ifdef DARKINTERVAL
	if (m_live_ragdoll_handle != NULL)
	{
		UTIL_Remove(m_live_ragdoll_handle);
	}
#endif
}
...
#ifdef DARKINTERVAL
	// special case for zombie workers, because they otherwise become too non-threatening in DI's first chapters
	if (IsOnFire() && !FClassnameIs(this, "npc_zombie_worker"))
	{
		if (pTarget->IsPlayer())
			return D_FR; // zombies that are ignited immediately stop being a direct threat
		else
			return ((RandomFloat(0, 1) < 0.7) ? D_FR : D_HT); // can still occasonally attack other things in the vicinity
	}
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CNPC_BaseZombie::IsValidEnemy(CBaseEntity *pEnemy)
{
...
bool CNPC_BaseZombie::CanPlayMoanSound()
{
	...
	if( random->RandomInt( 1, 1 * (s_iAngryZombies/2) ) == 1 )
	{
		return true;
	}
...
#ifdef DARKINTERVAL
		ENVELOPE_CONTROLLER.Play( m_pMoanSound, 1.0, 100 );
#else
		ENVELOPE_CONTROLLER.Play(m_pMoanSound, 1.0, m_flMoanPitch);
#endif
	}
#ifdef DARKINTERVAL
	envDefaultZombieMoanVolumeFast[ 1 ].durationMin = 0.1;
	envDefaultZombieMoanVolumeFast[ 1 ].durationMax = 0.4;
#else
...
#endif
	m_flNextMoanSound = CURTIME + duration + 9999;
}
...
void CNPC_BaseZombie::DieChopped( const CTakeDamageInfo &info )
{
...
#ifdef DARKINTERVAL
	DispatchParticleEffect("blood_zombie_split", WorldSpaceCenter(), GetAbsAngles(), this);
#endif
...
#ifdef DARKINTERVAL // because DI's zombies can have several version of torso/leg models
	CBaseEntity *pLegGib;
	pLegGib = CreateRagGib(GetLegsModel(),
		GetAbsOrigin(), GetAbsAngles(), vecLegsForce, flFadeTime, ShouldIgniteZombieGib());
#else
	...
	if (pLegGib)
	{
		...
		{
			pLegGib->SetRenderMode(kRenderNone);
			CBaseEntity *pOldLegGib = pLegGib;
			pLegGib = CreateServerRagdoll(pOldLegGib->GetBaseAnimating(), 0, CTakeDamageInfo(this, this, 0, 0), COLLISION_GROUP_DEBRIS);
			UTIL_Remove(pOldLegGib);
		}
#endif
	}
	...
	if ( pTorsoGib )
	{
		...
		if (HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL))
		{
			pTorsoGib->SetRenderMode(kRenderNone);
			CBaseEntity *pOldTorsoGib = pTorsoGib;
			pTorsoGib = CreateServerRagdoll(pOldTorsoGib->GetBaseAnimating(), 0, CTakeDamageInfo(this, this, 0, 0), COLLISION_GROUP_DEBRIS);
			UTIL_Remove(pOldTorsoGib);
		}
#endif
	}
...
bool CNPC_BaseZombie::ShouldIgnite( const CTakeDamageInfo &info )
{
	...
	if ( info.GetDamageType() & DMG_BURN )
	{
		...
		m_flBurnDamageResetTime = CURTIME + 5;
...
#ifdef HL2_EPISODIC
	if ( /*HL2GameRules()->IsAlyxInDarknessMode() == true &&*/ GetEffectEntity() != NULL ) // DI CHANGE: always cast light on ignite
	{
...
CBaseEntity *CNPC_BaseZombie::ClawAttack( float flDist, int iDamage, QAngle &qaViewPunch, Vector &vecVelocityPunch, int BloodOrigin  )
{
	...
	{
		m_hPhysicsEnt = NULL;
		m_flNextSwat = CURTIME + random->RandomFloat( 2, 4 );
	}
...
void CNPC_BaseZombie::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ( pEvent->event == AE_ZOMBIE_SWATITEM )
	{
		...
		if ( pEnemy )
		{
			...
			m_flNextSwatScan = CURTIME + ZOMBIE_SWAT_DELAY;
...
void CNPC_BaseZombie::Spawn( void )
{
	...
	m_flNextSwatScan = CURTIME;
	m_pMoanSound = NULL;

	m_flNextMoanSound = CURTIME + 9999;
...
#ifdef DARKINTERVAL
		|| GetActivity() == ACT_ZOMBIE_SLUMP_A_RISE
		|| GetActivity() == ACT_ZOMBIE_SLUMP_B_RISE
#endif
		)
	{
...
#ifndef HL2_EPISODIC
	else if ( m_flNextFlinch >= CURTIME )
	{
		ClearCustomInterruptCondition( COND_LIGHT_DAMAGE );
		ClearCustomInterruptCondition( COND_HEAVY_DAMAGE );
	}
...
void CNPC_BaseZombie::OnScheduleChange( void )
{
	...
	{
		m_flNextFlinch = CURTIME + ZOMBIE_FLINCH_DELAY;
	} 
...
#ifdef DARKINTERVAL // more convenient than scripted sequences or actbusy
	if (m_zombie_starts_slumped_int > SLUMP_NONE && !IsInAScript())
	{
		if (m_zombie_starts_slumped_int == SLUMP_RAGDOLL
			&& !IsCurSchedule(SCHED_DI_FALL_IN_RAGDOLL_PUSH))
		{
			m_flExtraRagdollTime = -1;
			return SCHED_DI_FALL_IN_RAGDOLL_PUSH;
		}
		return SCHED_ZOMBIE_SLUMP_WAIT;
	}

	if ((IsSlumped() && m_zombie_starts_slumped_int > SLUMP_NONE) || m_zombie_starts_slumped_int == SLUMP_RAGDOLL) // only consider this if we began as a slumping zombie because the mapper intended it.
	{
		if (HasCondition(COND_LIGHT_DAMAGE) || HasCondition(COND_HEAVY_DAMAGE))
		{
			if (m_zombie_starts_slumped_int == SLUMP_RAGDOLL)
			{
				m_flExtraRagdollTime = 0;
				return SCHED_COMBAT_FACE;
			}
			return SCHED_ZOMBIE_SLUMP_RISE;
		}
		...
		if (HasCondition(COND_CAN_MELEE_ATTACK1) || HasCondition(COND_CAN_MELEE_ATTACK1))
			if (m_zombie_starts_slumped_int == SLUMP_RAGDOLL)
			{
				m_flExtraRagdollTime = 0;
				return SCHED_COMBAT_FACE;
...
#ifndef DARKINTERVAL // this wasn't a functioning thing anyway
...
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::InputEnterSlumpMode(inputdata_t &inputdata)
{
	if (IsSlumped())
		return;

	if (inputdata.value.Int()/*slump mode int*/ == SLUMP_NONE)
	{
		Msg("wrong input from EnterSlumpMode! Shoudl be integer between 1 and 3!\n");
		return;
	}

	m_zombie_starts_slumped_int = inputdata.value.Int(); // when told to do this via input, drop on the floor by default
	
	ClearSchedule("forced via input");
	if (m_zombie_starts_slumped_int == SLUMP_RAGDOLL) // this'll get handled automatically in SelectSchedule()
	{
		return;
	}
	SetSchedule(SCHED_ZOMBIE_SLUMP_WAIT);
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CNPC_BaseZombie::InputExitSlumpMode(inputdata_t &inputdata)
{
	if (!IsSlumped())
	{
		if (m_zombie_starts_slumped_int == SLUMP_RAGDOLL)
		{
			m_flExtraRagdollTime = 0; 
			m_zombie_starts_slumped_int = SLUMP_NONE;
		}		
		return;
	}

	SetSchedule(SCHED_ZOMBIE_SLUMP_RISE);
	m_zombie_starts_slumped_int = SLUMP_NONE; // stop trying to go to slump anymore
}
#endif // DARKINTERVAL
//---------------------------------------------------------
//---------------------------------------------------------
bool CNPC_BaseZombie::IsSlumped( void )
{
...
#endif
	{
		...
		if ( IsCurSchedule(SCHED_ZOMBIE_SLUMP_WAIT) ) return true;
	}
	return false;
}
...
void CNPC_BaseZombie::GatherConditions( void )
{
	...
	if( m_NPCState == NPC_STATE_COMBAT && !m_fIsTorso )
	{
		...
		if( CURTIME >= m_flNextSwatScan && (m_hPhysicsEnt == NULL) )
		{
			FindNearestPhysicsObject( ZOMBIE_MAX_PHYSOBJ_MASS );
			m_flNextSwatScan = CURTIME + 2.0;
		}
	}

	if( (m_hPhysicsEnt != NULL) && CURTIME >= m_flNextSwat && HasCondition( COND_SEE_ENEMY ) && !HasCondition( COND_ZOMBIE_RELEASECRAB ) )
	{
...
void CNPC_BaseZombie::PrescheduleThink( void )
{
	...
	//
	if ( ( m_flBurnDamageResetTime ) && ( CURTIME >= m_flBurnDamageResetTime ) )
	{
		m_flBurnDamage = 0;
	}
}
...
void CNPC_BaseZombie::StartTask( const Task_t *pTask )
{
	...
	break;
#ifdef DARKINTERVAL // more convenient than scripted sequences or actbusy
	case TASK_ZOMBIE_SLUMP_WAIT:
	{
		if (m_zombie_starts_slumped_int == SLUMP_WALL )
			SetActivity((Activity)ACT_ZOMBIE_SLUMP_A );
		else if (m_zombie_starts_slumped_int == SLUMP_FLOOR )
			SetActivity((Activity)ACT_ZOMBIE_SLUMP_B );
		TaskComplete();
		return;
	}
	break;

	case TASK_ZOMBIE_SLUMP_RISE:
	{
		if( m_zombie_starts_slumped_int == SLUMP_WALL || GetCurrentActivity() == (Activity)ACT_ZOMBIE_SLUMP_A )
			SetActivity((Activity)ACT_ZOMBIE_SLUMP_A_RISE);
		else if (m_zombie_starts_slumped_int == SLUMP_FLOOR || GetCurrentActivity() == (Activity)ACT_ZOMBIE_SLUMP_B)
			SetActivity((Activity)ACT_ZOMBIE_SLUMP_B_RISE);
	}
	break;
#endif // DARKINTERVAL
	default:
...
void CNPC_BaseZombie::RunTask( const Task_t *pTask )
{
	...
	case TASK_ZOMBIE_SLUMP_WAIT:
	{
		if (HasCondition(COND_CAN_RANGE_ATTACK1) 
			...
			|| HasCondition(COND_ZOMBIE_RELEASECRAB)
...
void CNPC_BaseZombie::BecomeTorso( const Vector &vecTorsoForce, const Vector &vecLegsForce )
{
...
#ifdef DARKINTERVAL
		DispatchParticleEffect("blood_zombie_split", origin, GetAbsAngles(), this);
#endif
	}
	...
	if ( m_fIsTorso == true )
	{
		...
		if ( pGib )
		{
			...
			if (HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL))
			{
				pGib->SetRenderMode(kRenderNone);
				CBaseEntity *pOldGib = pGib;
				pGib = CreateServerRagdoll(pOldGib->GetBaseAnimating(), 0, CTakeDamageInfo(this, this, 0, 0), COLLISION_GROUP_DEBRIS);
				UTIL_Remove(pOldGib);
			}
#else
			// normally, HL2 sets the owner so torso won't collide with the legs. But we want that! We want to beat up the torso with his own legs!
...
{
#ifdef DARKINTERVAL
	if( random->RandomInt( 1, 4 * s_iAngryZombies ) == 1 )
#else	
...
#define ZOMBIE_CRAB_INHERITED_SPAWNFLAGS	(SF_NPC_GAG|SF_NPC_LONG_RANGE|SF_NPC_FADE_CORPSE|SF_NPC_ALWAYSTHINK|SF_ALWAYS_SERVER_RAGDOLL)
...
void CNPC_BaseZombie::ReleaseHeadcrab( const Vector &vecOrigin, const Vector &vecVelocity, bool fRemoveHead, bool fRagdollBody, bool fRagdollCrab )
{
	...
	else
	{
		...
		pCrab->m_flNextAttack = CURTIME + 1.0f;
...
{
	if ( pMoveGoal->directTrace.fStatus == AIMR_BLOCKED_ENTITY && CURTIME >= m_flNextSwat )
	{
...
#ifdef DARKINTERVAL
	DEFINE_FIELD(m_live_ragdoll_handle, FIELD_EHANDLE),

	DEFINE_KEYFIELD(m_zombie_starts_slumped_int, FIELD_INTEGER, "startslumped"),

	DEFINE_INPUTFUNC(FIELD_INTEGER, "EnterSlumpMode", InputEnterSlumpMode),
	DEFINE_INPUTFUNC(FIELD_VOID, "ExitSlumpMode", InputExitSlumpMode),
#endif
END_DATADESC()
...
AI_BEGIN_CUSTOM_NPC( base_zombie, CNPC_BaseZombie )
...
#ifdef DARKINTERVAL // more convenient than scripted sequences or actbusy
	DECLARE_TASK( TASK_ZOMBIE_SLUMP_WAIT )
	DECLARE_TASK( TASK_ZOMBIE_SLUMP_RISE )
#endif
...
#ifdef DARKINTERVAL // more convenient than scripted sequences or actbusy
	DECLARE_ACTIVITY( ACT_ZOMBIE_SLUMP_A )
	DECLARE_ACTIVITY( ACT_ZOMBIE_SLUMP_A_RISE )
	DECLARE_ACTIVITY( ACT_ZOMBIE_SLUMP_B )
	DECLARE_ACTIVITY( ACT_ZOMBIE_SLUMP_B_RISE )
#endif
	...
	DEFINE_SCHEDULE
	(
		SCHED_ZOMBIE_SLUMP_WAIT,

		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_ZOMBIE_SLUMP_RISE"
		"		TASK_ZOMBIE_SLUMP_WAIT			1"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
		"		COND_LIGHT_DAMAGE"
	...
	DEFINE_SCHEDULE
	(
		SCHED_ZOMBIE_SLUMP_RISE,

		"	Tasks"
		"		TASK_SET_FAIL_SCHEDULE	SCHEDULE:SCHED_ZOMBIE_SLUMP_WAIT"
		"		TASK_ZOMBIE_SLUMP_RISE	1"
		""
		"	Interrupts"
		"		COND_TASK_FAILED"
	)
