...
#include "ai_memory.h"
#include "npc_helicopter.h"

#ifndef DARKINTERVAL
#ifdef HL2_EPISODIC
#define HELICOPTER_EPISODIC
#endif
#endif // DARKINTERVAL

#ifdef HELICOPTER_EPISODIC
#include "physics_bone_follower.h"
#endif // HELICOPTER_EPISODIC
...
#ifdef DARKINTERVAL // missile attachments, different blades, different VMTs
#define CHOPPER_MODEL_NAME	"models/_Monsters/Combine/combine_helicopter.mdl"
#else
...
// CVars
#ifdef DARKINTERVAL // to make it possible to kill with regular weapons and explosives
ConVar	sk_helicopter_health( "sk_helicopter_health","1000");
#else
...
#ifdef DARKINTERVAL // input-based missile attack
ConVar  sk_helicopter_missile_dmg("sk_helicopter_missile_dmg", "35", 0, FCVAR_NONE);
ConVar  sk_helicopter_missile_radius("sk_helicopter_missile_dmg", "128", 0, FCVAR_NONE);
#endif
...
#ifdef DARKINTERVAL // input-based missile attack
static const char* s_pMissilesThinkContext = "Missiles";
#endif
...
#ifdef DARKINTERVAL // input-based missile attack
	void LaunchHomingMissile(CBaseEntity* target, bool attachment);
	void InputLaunchMissiles(inputdata_t &inputdata);
	void LaunchMissiles(const char *pszTargetString);
	void FireMissilesThink(); // context think
	bool m_last_missile_launch_bool; // don't go in datadesc, no need
	int m_missiles_to_launch_int;
	EHANDLE m_missiles_target_handle;
#endif
	// Drop those bombs!
	void DropBombs( );
...
#ifdef DARKINTERVAL	// DI NEW - let us specify pilot corpse specifics
	int			m_special_death_int;

	// DI NEW - outputs, used for example for CH06 OUTERWALL responses
	COutputEvent m_OnLaunchMissile;
	COutputEvent m_OnDropBomb;
	COutputEvent m_OnChargeBurst;
	COutputEvent m_OnStartBurst;
	COutputEvent m_OnEndBurst;
	COutputEvent m_OnStartNormalBurst;
	COutputEvent m_OnStartLongBurst;
	COutputEvent m_OnStartFastBurst;
	COutputEvent m_OnStartContinuousBurst;
#endif
};
...
#ifdef DARKINTERVAL // redundant names
LINK_ENTITY_TO_CLASS(npc_attackhelicopter, CNPC_AttackHelicopter);
LINK_ENTITY_TO_CLASS(npc_combinehelicopter, CNPC_AttackHelicopter);
LINK_ENTITY_TO_CLASS(npc_chopper, CNPC_AttackHelicopter);
LINK_ENTITY_TO_CLASS(npc_hunterchopper, CNPC_AttackHelicopter);
#endif
...
#ifdef DARKINTERVAL // for Fb death
	DEFINE_KEYFIELD( m_special_death_int, FIELD_INTEGER, "specialDeath" ),
#endif
...
#ifdef DARKINTERVAL // input-based missile attack
	DEFINE_INPUTFUNC( FIELD_STRING, "LaunchMissilesAt", InputLaunchMissiles ),
	DEFINE_FIELD( m_missiles_target_handle, FIELD_EHANDLE),
	DEFINE_FIELD( m_missiles_to_launch_int, FIELD_INTEGER),
	DEFINE_THINKFUNC(FireMissilesThink),
#endif
...
#ifdef DARKINTERVAL // input-based missile attack
	DEFINE_OUTPUT( m_OnLaunchMissile, "OnLaunchMissile"),
	DEFINE_OUTPUT( m_OnDropBomb, "OnDropBomb"),
	DEFINE_OUTPUT( m_OnChargeBurst, "OnChargeBurst"),
	DEFINE_OUTPUT( m_OnStartBurst, "OnStartBurst"),
	DEFINE_OUTPUT( m_OnEndBurst, "OnEndBurst"),
	DEFINE_OUTPUT( m_OnStartNormalBurst, "OnStartNormalBurst"),
	DEFINE_OUTPUT( m_OnStartLongBurst, "OnStartLongBurst"),
	DEFINE_OUTPUT( m_OnStartFastBurst, "OnStartFastBurst"),
	DEFINE_OUTPUT( m_OnStartContinuousBurst, "OnStartContinuousBurst"),
#endif
END_DATADESC()
...
#ifdef DARKINTERVAL
	,m_special_death_int(0),
	m_missiles_to_launch_int(0)
#endif
{
	m_flMaxSpeed = 0;
}
...
#ifdef DARKINTERVAL // for Fb death
		PrecacheModel("models/_Monsters/Combine/combine_soldier.mdl");
#endif
	}
...
#ifdef DARKINTERVAL // input-based missile attack
	m_missiles_target_handle.Set(NULL);
#endif
}
...
void CNPC_AttackHelicopter::Startup()
{
	...
	if ( HasSpawnFlags( SF_HELICOPTER_LIGHTS ) )
	{
		...
		SetContextThink( &CNPC_AttackHelicopter::BlinkLightsThink, CURTIME + CHOPPER_LIGHT_BLINK_TIME_SHORT, s_pBlinkLightThinkContext );
	}
}
...
void CNPC_AttackHelicopter::BlinkLightsThink()
{
	...
	SetContextThink( &CNPC_AttackHelicopter::BlinkLightsThink, CURTIME + flTime, s_pBlinkLightThinkContext );
}
...
void CNPC_AttackHelicopter::SpotlightShutdown()
{
	...
	SetContextThink( NULL, CURTIME, s_pSpotlightThinkContext );
}
...
void CNPC_AttackHelicopter::SpotlightThink()
{
	...
	m_Spotlight.Update();
	SetContextThink( &CNPC_AttackHelicopter::SpotlightThink, CURTIME + TICK_INTERVAL, s_pSpotlightThinkContext );
}
...
bool CNPC_AttackHelicopter::DoGunIdle( const Vector &vGunDir, const Vector &vTargetDir )
{
	...
	else
	{
		...
		m_flChargeTime = CURTIME + random->RandomFloat(flChargeTime - flVariance, flChargeTime + flVariance);
		m_nGunState = GUN_STATE_CHARGING;
		m_flAvoidMetric = 0.0f;
		m_vecLastAngVelocity.Init( 0, 0, 0 );
#ifdef DARKINTERVAL
...
bool CNPC_AttackHelicopter::DoGunCharging( )
{
	...
	case SHOOT_MODE_DEFAULT:
#ifdef DARKINTERVAL
		{
			...
			m_OnStartBurst.FireOutput(this, this);
		}
		break;
#endif
...
#ifdef DARKINTERVAL
			m_OnStartBurst.FireOutput(this, this);
			m_OnStartFastBurst.FireOutput(this, this);
#endif
		}
		break;
...
#ifdef DARKINTERVAL
			m_OnStartBurst.FireOutput(this, this);
			m_OnStartLongBurst.FireOutput(this, this);
#endif
		}
		break;
...
#ifdef DARKINTERVAL
		m_OnStartBurst.FireOutput(this, this);
		m_OnStartContinuousBurst.FireOutput(this, this);
#endif
		break;
	}
...
void CNPC_AttackHelicopter::FireElectricityGun( )
{
	...
	m_flNextAttack = CURTIME + random->RandomFloat( 0.3f, 1.0f );
}
...
bool CNPC_AttackHelicopter::DoGunFiring( const Vector &vBasePos, const Vector &vGunDir, const Vector &vecFireAtPosition )
{
	...
	m_flNextAttack = CURTIME + m_flIdleTimeDelay + random->RandomFloat(flIdleTime - flVariance, flIdleTime + flVariance);
	m_nGunState = GUN_STATE_IDLE;
	SetPauseState( PAUSE_NO_PAUSE );
#ifdef DARKINTERVAL
	m_OnEndBurst.FireOutput(this, this);
#endif
	return true;
}
#ifdef DARKINTERVAL // input-based missile attack
//------------------------------------------------------------------------------
// Purpose:  Create a homing missile, send it toward the target.
// Input: target = homing target, attachment = left or right wing
// attachment, bool is used to flip between the two
//------------------------------------------------------------------------------
#define MISSILE_SPEED 1500
void CNPC_AttackHelicopter::LaunchHomingMissile(CBaseEntity *target, bool attachment)
{
	Vector vecMissileStart;

	int missileAttachment;
	if (attachment == 0) missileAttachment = LookupAttachment("missile_launch_left");
	else  missileAttachment = LookupAttachment("missile_launch_right");

	GetAttachment(missileAttachment, vecMissileStart, NULL);
	
	Vector forward;
	GetVectors(&forward, NULL, NULL);
	
	Vector vecVelocity;
	VectorMultiply(forward, MISSILE_SPEED, vecVelocity);

	QAngle angles;
	VectorAngles(forward, angles);

	CGrenadeHomer *pGrenade = CGrenadeHomer::CreateGrenadeHomer(MAKE_STRING("models/weapons/w_missile_launch.mdl"), NULL_STRING, vecMissileStart, vec3_angle, edict());
	pGrenade->Spawn();
	pGrenade->SetHoming((0.01 * 40),
		...
		1.0f, // Duration
		1.0f);// Ramp down
	pGrenade->SetDamage(sk_helicopter_missile_dmg.GetFloat());
	pGrenade->SetDamageRadius(sk_helicopter_missile_radius.GetFloat());
	...
	m_OnLaunchMissile.FireOutput(this, this);
}

void CNPC_AttackHelicopter::LaunchMissiles(const char *pszTargetString)
{
	// Parse the keyvalue data
	char parseString[1000];
	Q_strncpy(parseString, pszTargetString, sizeof(parseString));

	// Look for an entity string
	char *entityString = strtok(parseString, " ");
	if (entityString != NULL)
	{
		// Get the missiles amount
		char *missilesString = strtok(NULL, " ");
		int numToFire = (missilesString) ? atoi(missilesString) : 1;
		//Msg("target is %s, num is %i\n", entityString, numToFire);
		...
		if( entity )
		{
			// we have entity name and number of missiles. Fire. 
			bFoundEntity = true;
			m_missiles_to_launch_int = numToFire;
			m_missiles_target_handle = entity;
			SetContextThink(&CNPC_AttackHelicopter::FireMissilesThink, CURTIME + 0.1f, s_pMissilesThinkContext);
		}
		else
		{
			Warning("Can't parse helicopter missiles input (%s) - Expecting '[name] [number of missiles]'\n", pszTargetString);
			Assert(0);
			return;
		}
		...
		if (!bFoundEntity)
		{
			...
			if (!stricmp("player", entityString) || !stricmp("!player", entityString))
			{
				...
				m_missiles_target_handle = UTIL_GetLocalPlayer();
				SetContextThink(&CNPC_AttackHelicopter::FireMissilesThink, CURTIME + 0.1f, s_pMissilesThinkContext);
			}
		}
#endif
...
void CNPC_AttackHelicopter::InputLaunchMissiles(inputdata_t &inputdata)
{
	LaunchMissiles(inputdata.value.String());
}

//------------------------------------------------------------------------------
// Fire the salvo, alternating between wing launchers
//------------------------------------------------------------------------------
void CNPC_AttackHelicopter::FireMissilesThink()
{
	StudioFrameAdvance();

	if (m_missiles_target_handle == NULL)
		return;
	
	LaunchHomingMissile(m_missiles_target_handle, m_last_missile_launch_bool);

	m_last_missile_launch_bool = !m_last_missile_launch_bool;

	EmitSound("PropAPC.FireRocket");

	m_missiles_to_launch_int--;

	if (m_missiles_to_launch_int <= 0)
	{
		m_missiles_target_handle = NULL; // forget the target
		SetContextThink(NULL, 0, s_pMissilesThinkContext);
	}
	else
		SetContextThink(&CNPC_AttackHelicopter::FireMissilesThink, CURTIME + 0.5f, s_pMissilesThinkContext);
}
#endif // DARKINTERVAL
//------------------------------------------------------------------------------
// Is it "fair" to drop this bomb?
...
#endif // HELICOPTER_EPISODIC
#ifdef DARKINTERVAL
	m_OnDropBomb.FireOutput(this, this);
#endif
	return pGrenade;
}
...
{
	if ( m_flInputDropBombTime > CURTIME )
		return;

	// Prevent two triggers from being hit the same frame
	m_flInputDropBombTime = CURTIME + 0.01f;
	...
	if ( ShouldDropBombs() )
	{
		m_flNextAttack = CURTIME + 0.5f + random->RandomFloat( 0.3f, 0.6f );
	}
}
...
void CNPC_AttackHelicopter::InputDropBombAtTargetInternal( inputdata_t &inputdata, bool bCheckFairness )
{
	...
	if ( ShouldDropBombs() )
	{
		m_flNextAttack = CURTIME + 1.5f + random->RandomFloat( 0.1f, 0.2f );
	}
}
...
{
	m_flInputDropBombTime = CURTIME + inputdata.value.Float();
...
bool CNPC_AttackHelicopter::ShouldDropBombs( void )
{
	...
	{
		m_flLastFastTime = CURTIME;
	}
	else
	{
		if ( ( CURTIME - m_flLastFastTime ) < BOMB_GRACE_PERIOD )
			return false;
	}
...
{
	if ( CURTIME < m_flNextBullrushBombTime )
		return;
	...
	m_flNextBullrushBombTime = CURTIME + 0.2f;
}
...
bool CNPC_AttackHelicopter::FireGun( void )
{
	...
	{
		if ( (m_flLastSeen + 1 <= CURTIME) || (m_flPrevSeen + m_flGracePeriod > CURTIME) )
			return false;
	}
...
void CNPC_AttackHelicopter::DropCorpse( int nDamage )
{
	...
	if( nDamage > 250.0f )
		nDamage = 250.0f;
#ifdef DARKINTERVAL // for Fb death
	if (m_special_death_int == 0)
		m_flLastCorpseFall = CURTIME + 3.0;
	else if (m_special_death_int == 1)
		m_flLastCorpseFall = CURTIME + FLT_MAX; // only one corpse for Fb's heli
#else
	m_flLastCorpseFall = CURTIME + 3.0;
#endif
	...
	{
		CBaseEntity *pGib = CreateRagGib("models/_Monsters/Combine/combine_soldier.mdl", GetAbsOrigin(), GetAbsAngles(), vecForceVector);
		if (pGib)
		{
		//	pGib->SetOwnerEntity(this);
			CBaseAnimating *pGibAnim = assert_cast<CBaseAnimating*>(pGib);
			pGibAnim->SetName(AllocPooledString("fb_corpse"));
			pGibAnim->SetNameplateText(MAKE_STRING("FAULKENBURY"));
			pGibAnim->m_nSkin = 4;
			pGibAnim->SetBodygroup(2, 1);
			pGibAnim->RemoveSpawnFlags(SF_NPC_FADE_CORPSE);
			pGibAnim->SetFadeDistance(FLT_MAX, FLT_MAX); // this disables SUB_AllowedToFade
		}
	}
#else
	CBaseEntity *pGib = CreateRagGib("models/combine_soldier.mdl", GetAbsOrigin(), GetAbsAngles(), vecForceVector);
...
void CNPC_AttackHelicopter::TraceAttack( const CTakeDamageInfo &info, const Vector &vecDir, trace_t *ptr )
{
	...
	if ( ( info.GetDamageType() & DMG_AIRBOAT ) || 
		...
		&& GetAmmoDef()->ArmorMitigation(info.GetAmmoType()) >= 1.5)
#endif
		)
	{
...
{
	// We don't take blast damage from anything but the airboat or missiles (or myself!) // DI CHANGE: or armour-piercing weapons
	if (info.GetInflictor() != this)
	{
#ifdef DARKINTERVAL // to make it possible to kill with regular weapons and explosives
		if ((info.GetDamageType() & DMG_BULLET) 
			&& GetAmmoDef()->ArmorMitigation(info.GetAmmoType()) >= 1.5) // oicw has 1.5 armor mitig. 357 has 2. Tau has 2.5
		{
			CTakeDamageInfo scaledInfo = info;
			scaledInfo.ScaleDamage(0.5f); // DI todo - consider adding armour mitigation coeff to scaling formula?
			return BaseClass::OnTakeDamage(scaledInfo);
		}
#endif
...
#ifdef DARKINTERVAL // DI CHANGE: makes helicopter push back from blast damage accordingly to its set Phys Damage Scale
		ApplyAbsVelocityImpulse( damageDir * 500.0f * m_impactEnergyScale );
#else
...
#if (defined HELICOPTER_EPISODIC) || (defined DARKINTERVAL) // this particular change we want to inherit
...
#ifdef DARKINTERVAL // for Fb death
	if (m_special_death_int == 1) // single guaranteed corpse in case of Fb's heli.
	{
		DropCorpse(info.GetDamage());
	}
#endif
...
float CNPC_AttackHelicopter::UpdatePerpPathDistance( float flMaxPathOffset )
{
	...
	if ( ShouldDropBombs() )
	{
		...
		flNewPathOffset += flSinHeight * sin( 2.0f * M_PI * (CURTIME / 6.0f) );
	}
	...
	float flMaxChange = 1000.0f * (CURTIME - GetLastThink());
...
void CNPC_AttackHelicopter::UpdateFacingDirection( const Vector &vecActualDesiredPosition )
{
	...
	bool bSeenTargetRecently = HasSpawnFlags( SF_HELICOPTER_AGGRESSIVE ) || ( m_flLastSeen + 5 > CURTIME ); 
...
#ifdef DARKINTERVAL // hack to make helicopters fly facing their path if they don't have enemies (they face map origin otherwise)
	if (!GetEnemy() && HasSpawnFlags(SF_ALWAYS_SERVER_RAGDOLL)) CurrentPathDirection(&m_vecDesiredFaceDir);
#endif
...
inline void CNPC_AttackHelicopter::SetSecondaryMode( int nMode, bool bRetainTime )
{
	...
	if (!bRetainTime)
	{
		m_flSecondaryModeStartTime = CURTIME;
	}
}
...
{
	return CURTIME - m_flSecondaryModeStartTime;
}
	...
	case BULLRUSH_MODE_GET_DISTANCE:
		{
			...
			if ( GetHealth() <= m_flNextMegaBombHealth )
			{
				...
				m_flNextBullrushBombTime = CURTIME;
...
void CGrenadeHelicopter::Spawn( void )
{
	...
	if( hl2_episodic.GetBool() )
	{
		SetContextThink( &CGrenadeHelicopter::AnimateThink, CURTIME, s_pAnimateThinkContext );
	}
}
...
void CGrenadeHelicopter::BecomeActive()
{
	...
	if ( hl2_episodic.GetBool() )
	{
		...
		{
			SetNextThink( CURTIME + GetBombLifetime() );
		}
		else
		{
	...
	if ( !bMegaBomb )
	{
		SetContextThink( &CGrenadeHelicopter::RampSoundThink, CURTIME + GetBombLifetime() - BOMB_RAMP_SOUND_TIME, s_pRampSoundContext );
	...
	SetContextThink( &CGrenadeHelicopter::WarningBlinkerThink, CURTIME + (GetBombLifetime() - 2.0f), s_pWarningBlinkerContext );

#ifdef HELICOPTER_EPISODIC
	m_flBlinkFastTime = CURTIME + GetBombLifetime() - 1.0f;
#endif//HELICOPTER_EPISODIC
}
...
void CGrenadeHelicopter::RampSoundThink( )
{
	...
	SetContextThink( NULL, CURTIME, s_pRampSoundContext );
}
...
#ifdef HELICOPTER_EPISODIC
	if( CURTIME >= m_flBlinkFastTime )
	{
		SetContextThink( &CGrenadeHelicopter::WarningBlinkerThink, CURTIME + 0.1f, s_pWarningBlinkerContext );
	}
	else
	{
		SetContextThink( &CGrenadeHelicopter::WarningBlinkerThink, CURTIME + 0.2f, s_pWarningBlinkerContext );
	}
#endif//HELICOPTER_EPISODIC
}
...
{
	StudioFrameAdvance();
	SetContextThink( &CGrenadeHelicopter::AnimateThink, CURTIME + 0.1f, s_pAnimateThinkContext );
}
...
#ifndef HELICOPTER_EPISODIC	// in ep2, don't do this here, do it in Touch()
...
void CGrenadeHelicopter::DoExplosion( const Vector &vecOrigin, const Vector &vecVelocity )
{
	...
	ExplosionCreate( GetAbsOrigin(), GetAbsAngles(), GetOwnerEntity() ? GetOwnerEntity() : this, sk_helicopter_grenadedamage.GetFloat(), 
		sk_helicopter_grenaderadius.GetFloat(), (SF_ENVEXPLOSION_NOSPARKS | SF_ENVEXPLOSION_NODECAL|SF_ENVEXPLOSION_NOFIREBALL|SF_ENVEXPLOSION_NOPARTICLES), 
...
CHelicopterChunk *CHelicopterChunk::CreateHelicopterChunk( const Vector &vecPos, const QAngle &vecAngles, const Vector &vecVelocity, const char *pszModelName, int chunkID )
{
	...
	pChunk->SetNextThink( CURTIME + 0.1f );
