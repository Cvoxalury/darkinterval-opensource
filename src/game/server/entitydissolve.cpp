...
ConVar dissolve_fade_in_end_time("dissolve_fade_in_end_time", "1.0");
ConVar dissolve_fade_out_start_time("dissolve_fade_out_start_time", "2.0");
...
// Lifetime 
//-----------------------------------------------------------------------------
#if 0 //def DARKINTERVAL // todo - turn into keyfields?
#define DISSOLVE_FADE_IN_START_TIME			(dissolve_fade_in_start_time.GetFloat())
#define DISSOLVE_FADE_IN_END_TIME			(dissolve_fade_in_end_time.GetFloat())
#define DISSOLVE_FADE_OUT_MODEL_START_TIME	(dissolve_fade_out_model_start_time.GetFloat())
#define DISSOLVE_FADE_OUT_MODEL_END_TIME	(dissolve_fade_out_model_end_time.GetFloat())
#define DISSOLVE_FADE_OUT_START_TIME		(dissolve_fade_out_start_time.GetFloat())
#define DISSOLVE_FADE_OUT_END_TIME			(dissolve_fade_out_end_time.GetFloat())
#else
...
#define DISSOLVE_SPRITE_BURN_NAME "custom_uni/dissolve_fire_spark.vmt"
//-----------------------------------------------------------------------------
// Save/load 
//-----------------------------------------------------------------------------
...
#ifdef DARKINTERVAL
	DEFINE_KEYFIELD(m_dissolve_spawn_ashes_bool, FIELD_BOOLEAN, "spawnashes"),
#endif
...
#ifdef DARKINTERVAL
	SendPropBool( SENDINFO( m_dissolve_spawn_ashes_bool)),
#endif
END_SEND_TABLE()
...
void CEntityDissolve::Spawn()
{
	...
	if ( (m_nDissolveType == ENTITY_DISSOLVE_ELECTRICAL) || (m_nDissolveType == ENTITY_DISSOLVE_ELECTRICAL_LIGHT) )
	{
		...
		{
			SetContextThink( &CEntityDissolve::ElectrocuteThink, CURTIME + 0.01f, s_pElectroThinkContext );
		}
	}
	...
	if (m_nDissolveType == ENTITY_DISSOLVE_BURN )
	{
		...
		m_flFadeOutStart = DISSOLVE_FADE_OUT_START_TIME * 2;
...
void CEntityDissolve::InputDissolve( inputdata_t &inputdata )
{
	...
	while ((pTarget = gEntList.FindEntityGeneric(pTarget, STRING(strTarget), this, inputdata.pActivator)) != NULL)
	{
		...
		if (pBaseAnim)
		{
			pBaseAnim->Dissolve( NULL, CURTIME, false, m_nDissolveType, GetAbsOrigin(), m_nMagnitude );
		}
	}
}
...
#ifdef DARKINTERVAL
	else if ((nDissolveType == ENTITY_DISSOLVE_BURN))
	{
		// Emit burning loop sound
		CPASAttenuationFilter filter(pTarget);
		pTarget->EmitSound(filter, pTarget->GetSoundSourceIndex(), "General.BurningObject");
		...
		if (pTarget->IsNPC() && pTarget->MyNPCPointer()->CanBecomeRagdoll())
		{
			...
			{
				CBasePlayer *pPlayer = UTIL_PlayerByIndex(1);
				CTakeDamageInfo ragdollInfo(pPlayer, pPlayer, 10000.0, DMG_BURN | DMG_REMOVENORAGDOLL );
				pTarget->TakeDamage(ragdollInfo);
			}
...
void CEntityDissolve::DissolveThink( void )
{
	...
	{
#ifdef DARKINTERVAL
		// Stop the burning loop sound
		CPASAttenuationFilter filter(pTarget);
		pTarget->EmitSound(filter, pTarget->GetSoundSourceIndex(), "General.StopBurning");
#endif
		// Necessary to cause it to do the appropriate death cleanup
...
void CEntityDissolve::ElectrocuteThink( void )
{
	...
	SetContextThink( &CEntityDissolve::ElectrocuteThink, CURTIME + random->RandomFloat( 0.1, 0.2f ), 
