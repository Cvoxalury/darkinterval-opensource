...
#include "const.h"
#include "coordsize.h"
#include "darkinterval\logic_keypad.h" // keypad
...
#include "world.h"
#include "worldsize.h"
//#include "darkinterval\logic_fusebox.h" // fusebox
...
#ifndef DARKINTERVAL // slight rebalance of armour
static ConVar old_armor( "player_old_armor", "0" );
#else
static ConVar sk_armor_ratio("sk_armor_ratio", "0.2");
static ConVar sk_armor_bonus("sk_armor_bonus", "0.7");
#endif
...
#ifdef DARKINTERVAL // accessibility option to disable tinnitus DSP
extern ConVar snd_disable_tinnitus;
#endif
...
#ifndef DARKINTERVAL // in DI always give secondary ammo. The reason HL2 had this check was for Xbox (cheat codes?) 
					// and wanted to "prevent players on XBox having Combine Balls and so forth in areas 
					// of the game that were not tested with these items" and it's nonsensical in DI.
				&& pWeapon->HasSecondaryAmmo()
#endif
				)
			{
...
#ifdef DARKINTERVAL
	DEFINE_FIELD(m_nParalysisLasting, FIELD_INTEGER),
	DEFINE_FIELD(m_nParalysisGone, FIELD_INTEGER),
#endif
...
#ifdef DARKINTERVAL // for driven vehicles
	DEFINE_FIELD( m_iVehicleIntegrity, FIELD_INTEGER),
#endif
...
int CBasePlayer::ShouldTransmit( const CCheckTransmitInfo *pInfo )
{
	...
	if ( IsEffectActive( EF_NODRAW ) || ( IsObserver() && ( CURTIME - m_flDeathTime > 0.5 ) && 
		( m_lifeState == LIFE_DEAD ) && ( CURTIME - m_flDeathAnimTime > 0.5 ) ) )
	{
...
int CBasePlayer::TakeHealth( float flHealth, int bitsDamageType )
{
	...
	char buf[256];
	Q_snprintf(buf, 256, "[%f] Player %s healed for %d with damagetype %X\n", CURTIME, GetDebugName(), healingTaken, bitsDamageType);
	ADD_DEBUG_HISTORY( HISTORY_PLAYER_DAMAGE, buf );

	return healingTaken;
#endif
}
#ifdef DARKINTERVAL // prototype SP respawn mechanic
void CBasePlayer::Revive(void)
{
	ForceRespawn();
}
#endif
...
void CBasePlayer::DamageEffect(float flDamage, int fDamageType)
{
	...
	else if (fDamageType & DMG_PLASMA)
	{
		// Blue screen fade
#ifdef DARKINTERVAL
		color32 purple = { 175,45,255,80 };
		UTIL_ScreenFade(this, purple, 0.5, 0.1, FFADE_IN);
#else
		color32 blue = { 0,0,255,100 };
		UTIL_ScreenFade(this, blue, 0.2, 0.4, FFADE_MODULATE);
#endif
		// Very small screen shake
		ViewPunch(QAngle(RandomFloat(-0.1,0.1), RandomFloat(-0.1,0.1), RandomFloat(-0.1,0.1)));
	...
	{
		// Blue screen fade
		color32 blue = {75,85,255,100};
		UTIL_ScreenFade( this, blue, 0.5, 0.1, FFADE_IN );
...
int CBasePlayer::OnTakeDamage( const CTakeDamageInfo &inputInfo )
{
...
#ifdef DARKINTERVAL
	flBonus = sk_armor_bonus.GetFloat(); //OLD_ARMOR_BONUS;
	flRatio = sk_armor_ratio.GetFloat(); //OLD_ARMOR_RATIO;
#else
	// Early out if there's no damage
	if ( !info.GetDamage() )
		return 0;
	...
	// Armor. 
	if (m_ArmorValue && !(info.GetDamageType() & (DMG_FALL | DMG_DROWN | DMG_PARALYZE | DMG_POISON | DMG_RADIATION)) )// armor doesn't protect against fall or drown damage!
	{
	...
	while (fTookDamage && (!ftrivial || g_pGameRules->Damage_IsTimeBased( bitsDamage ) ) && ffound && bitsDamage)
	{
		...
		{
			if (fmajor)
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
				SetSuitUpdate("HEVVOXNEW_MINORFRACTURE", true, SUIT_NEXT_IN_30SEC);	// minor fracture
#else
...
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			if (fmajor)
				SetSuitUpdate("HEVVOXNEW_MAJORFRACTURE", true, SUIT_NEXT_IN_30SEC);	// major fracture
			else
				SetSuitUpdate("HEVVOXNEW_MINORFRACTURE", true, SUIT_NEXT_IN_30SEC);	// minor fracture
#else
			if ( fmajor )
		...
		if (bitsDamage & DMG_BULLET)
		{
			...
			if (m_lastDamageAmount > 5)
				SetSuitUpdate("HEVVOXNEW_BLOODLOSS", true, SUIT_NEXT_IN_30SEC);	// blood loss detected
#else
			if ( m_lastDamageAmount > 5 )
			...
			else
			{
				if( RandomFloat(0,1) > 0.5)
					SetSuitUpdate("HEVVOXNEW_MINORLACERATION", true, SUIT_NEXT_IN_30SEC);	// minor laceration
			}
#endif
...
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			if (fmajor)
				SetSuitUpdate("HEVVOXNEW_MAJORLACERATION", true, SUIT_NEXT_IN_30SEC);	// major laceration
			else
				SetSuitUpdate("HEVVOXNEW_MINORLACERATION", true, SUIT_NEXT_IN_30SEC);	// minor laceration
#else
			if ( fmajor )
		...
		if (bitsDamage & DMG_SONIC)
		{
			if (fmajor)
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
				SetSuitUpdate("HEVVOXNEW_INTERNALBLEEDING", true, SUIT_NEXT_IN_1MIN);	// internal bleeding
#else
...
#endif
		{
			...
			if (bitsDamage & DMG_PARALYZE)
			{
				m_nParalysisLasting += info.GetDamage();
				m_tbdPrev = CURTIME;
				m_rgbTimeBasedDamage[itbd_Paralyze] = 0;
			}
#endif
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_BLOODTOXINS", true, SUIT_NEXT_IN_1MIN);	// blood toxins detected
#else
			SetSuitUpdate("!HEV_DMG3", false, SUIT_NEXT_IN_1MIN);	// blood toxins detected
#endif
#ifdef DARKINTERVAL
			bitsDamage &= ~( DMG_POISON );
#else
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_CHEMICALDETECTED", true, SUIT_NEXT_IN_30SEC);	// hazardous chemicals detected
#else
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_BIOHAZARD", true, SUIT_NEXT_IN_30SEC);	// biohazard detected
#else
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_RADIATIONDETECTED", true, SUIT_NEXT_IN_1MIN);	// radiation detected
#else
...
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
		SetSuitUpdate("HEVVOXNEW_MORPHINE", true, SUIT_NEXT_IN_30MIN);	// morphine shot
#else
	...
	if (fTookDamage && !ftrivial && fcritical && flHealthPrev < 75)
	{
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_NEARDEATH", true, SUIT_NEXT_IN_10MIN);	// near death
#else
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_HEALTHCRITICAL", true, SUIT_NEXT_IN_10MIN);	// health critical
#else
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_SEEKMEDICAL", true, SUIT_NEXT_IN_5MIN); //seek medical attention
#else
	...
	if (fTookDamage && g_pGameRules->Damage_IsTimeBased(info.GetDamageType()) && flHealthPrev < 75)
	{
		...
		{
#ifdef DARKINTERVAL // renamed suit sentences for easier tracking
			SetSuitUpdate("HEVVOXNEW_HEALTHDROPPING", true, SUIT_NEXT_IN_10MIN);	// health dropping
#else
...
#ifdef DARKINTERVAL // accessibility option to disable tinnitus DSP
	if (snd_disable_tinnitus.GetBool()) 
#endif
		effect = RandomInt(32,34); // deafen without ringing
...
void CBasePlayer::RemoveAllItems( bool removeSuit, bool removeWeapons)
{
	if (removeWeapons)
	{
		if (GetActiveWeapon())
...
// DI todo - add more animations, perhaps?
...
void CBasePlayer::WaterMove()
{
	...
	{	// fully under water
		...
		if (m_AirFinished < CURTIME && !(GetFlags() & FL_GODMODE) )		// drown!
		{
			if (m_PainFinished < CURTIME)
			{
...
#ifdef DARKINTERVAL // DI only reloads a save for dead players by pressing spacebar, so that alt tabbing, +using, etc doesn't cause it.
	int fAnyButtonDown = (m_nButtons & IN_JUMP);
#else
...
bool CBasePlayer::StartReplayMode( float fDelay, float fDuration, int iEntity )
{
	...
	m_fDelay = fDelay;
	m_fReplayEnd = CURTIME + fDuration;
...
bool CBasePlayer::IsValidObserverTarget(CBaseEntity * target)
{
	...
	{
		if ( (player->m_flDeathTime + DEATH_ANIMATION_TIME ) < CURTIME )
		{
...
void CBasePlayer::AdjustPlayerTimeBase( int simulation_ticks )
{
	...
	else // multiplayer // DI todo - unused code?
	{
...
void CBasePlayer::PhysicsSimulate( void )
{
	...
	if ( commandsToRun > 0 )
	{
		...
		if ( m_vecPlayerSimInfo.Count() > 0 )
		{
			...
			pi->m_flGameSimulationTime = CURTIME;
...
void CBasePlayer::PlayerRunCommand(CUserCmd *ucmd, IMoveHelper *moveHelper)
{
	...
	if( GetFlags() & FL_FROZEN || 
		(developer.GetInt() == 0 && gpGlobals->eLoadType == MapLoad_NewGame && CURTIME < 0.50 ) )
	{
		ucmd->forwardmove = 0;
		ucmd->sidemove = 0;
		ucmd->upmove = 0;
...
{						
	...
	fPreframeSpeed = GetLocalVelocity().Length2D();
#ifdef DARKINTERVAL // was a test to apply damage from smashing into walls
	if (fPostframeSpeed > 0 && (fPostframeSpeed - fPreframeSpeed) > 400)
		Msg("Player experiences sudden halt\n");
#endif
}
...
void CBasePlayer::CheckTimeBasedDamage() 
{
	...
	for (i = 0; i < CDMG_TIMEBASED; i++)
	{
		...
		if ( m_bitsDamageType & iDamage )
		{
			...
			case itbd_Paralyze:
#ifdef DARKINTERVAL
				/* // unused experiment, disregard
				if (m_nParalysisLasting > m_nParalysisGone)
				{
					int nDif = MIN(m_nParalysisLasting - m_nParalysisGone, 10);
					TakeHealth(nDif, DMG_GENERIC);
					m_nParalysisGone += nDif;
				}
...
void CBasePlayer::UpdateGeigerCounter( void )
{
	...
	if (CURTIME < m_flgeigerDelay)
		return;

	m_flgeigerDelay = CURTIME + GEIGERDELAY;
...
void CBasePlayer::CheckSuitUpdate()
{
	...
	if ( CURTIME >= m_flSuitUpdate && m_flSuitUpdate > 0)
	{
		...
		m_flSuitUpdate = CURTIME + SUITUPDATETIME;
		}
		else
...
void CBasePlayer::SetSuitUpdate(const char *name, int fgroup, int iNoRepeatTime)
{
	...
	if ( !IsSuitEquipped() )
		return;
#ifdef DARKINTERVAL // for beginning of CH04 WAREHOUES until suit power is restored
	// If turned off by map script, return
	if (GlobalEntity_GetState("no_suit_updates") == GLOBAL_ON)
		return;
#endif
	...
	for (i = 0; i < CSUITNOREPEAT; i++)
	{
		if (isentence == m_rgiSuitNoRepeat[i])
			...
			if (m_rgflSuitNoRepeatTime[i] < CURTIME)
				{
	...
	if (iNoRepeatTime)
	{
		...
		m_rgflSuitNoRepeatTime[iempty] = iNoRepeatTime + CURTIME;
	}
	...
	if (m_flSuitUpdate <= CURTIME)
	{
		if (m_flSuitUpdate == 0)
			// play queue is empty, don't delay too long before playback
			m_flSuitUpdate = CURTIME + SUITFIRSTUPDATETIME;
		else 
			m_flSuitUpdate = CURTIME + SUITUPDATETIME; 
	}
}
...
{
	fPostframeSpeed = GetLocalVelocity().Length2D();
	...
	#ifdef PORTAL_COMPILE // Portalgun view model stays up when holding an object -Jeep
					|| FClassnameIs( GetActiveWeapon(), "weapon_portalgun" ) 
	#endif //#ifdef PORTAL_COMPILE			
					) )
				{  
...
void CBasePlayer::PostThinkVPhysics( void )
{
	...
	{
		float maxSpeed = m_flMaxspeed > 0.0f ? m_flMaxspeed : 400.0f;
...
CBaseEntity *CBasePlayer::EntSelectSpawnPoint()
{
	...
	player = edict();
// DI note - this is unused code really, but maybe someone wants to play 
// some MP maps inside of DI for whatever reason. Let it happen.
...
//-----------------------------------------------------------------------------
void CBasePlayer::Spawn(bool placeOnPlayerStart)
{
 ...
 // only preserve the shadow flag
	...
	m_flgeigerDelay = CURTIME + 2.0;	// wait a few seconds until user-defined message registrations
												// are recieved by all clients
	// DI todo - shouldn't this be scaled with aspect ratio?
	...
	CreateViewModel();
#ifdef DARKINTERVAL // for model/RT-based HUD, unused atm
//	CreateViewModel(1);
//	if (GetViewModel(1))
//	{
//		GetViewModel(1)->SetModel("models/view/hud.mdl");
//		GetViewModel(1)->SetSwayEnabled(false); // hud viewmodel must always stay fixed in place
//	}
#endif
...
void CBasePlayer::Precache( void )
{
	BaseClass::Precache();
#ifdef DARKINTERVAL
	PrecacheScriptSound("NoSuit.Jump");
#endif
...
#ifdef DARKINTERVAL
	enginesound->PrecacheSentenceGroup("HEVVOXNEW");
#endif
...
#ifdef DARKINTERVAL
	PrecacheModel( "decals/footprint_wet.vmt" );
	PrecacheModel( "decals/footprint_wet_left.vmt" );
#endif
	...
	*/
	...
	if ( gInitHUD )
		m_fInitHUD = true;
#ifdef DARKINTERVAL // for model/RT-based HUD, unused atm
//	PrecacheModel("models/view/hud.mdl");
#endif
}
...
{
#ifndef DARKINTERVAL // prototype revival while keeping items. Unused atm
	RemoveAllItems( true );
#endif
...
#ifdef DARKINTERVAL // prototype revival while keeping items. Unused atm
	Spawn( false ); // don't teleport the player back on info_player_start
#else
	Spawn(); // don't teleport the player back on info_player_start
#endif
}
...
void CBasePlayer::CommitSuicide( bool bExplode /*= false*/, bool bForce /*= false*/ )
{
	...
	if ( m_fNextSuicideTime > CURTIME && !bForce )
		return;

	// don't let them suicide for 5 seconds after suiciding
	m_fNextSuicideTime = CURTIME + 5;
...
#ifdef DARKINTERVAL // reusable client material proxy stuff
	m_Local.m_WeaponProxyValueSlot01_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot02_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot03_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot04_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot05_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot06_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot07_float = 0.0f;
	m_Local.m_WeaponProxyValueSlot08_float = 0.0f;
#endif
...
#ifdef DARKINTERVAL // DI NEW - without this check, all vehicles redeploy weapons that we might want to stay hidden 
					// (such as the slideshow pod in Eli's map)
	if (pVehicle->IsPassengerUsingStandardWeapons(nRole) == false && !m_Local.m_suppressing_weapons_bool)
#endif
	{
...
void CBasePlayer::ImpulseCommands( )
{
	...
	switch (iImpulse)
	{
#ifdef DARKINTERVAL // sprint toggling
	case 99:
		if (ToggleSprintIsOn())
		{
			ToggleSprintTurnOff();
...
#ifdef DARKINTERVAL
void CBasePlayer::PlacePlayerFootprint()
{
	unsigned char mType = GetSurfaceData()->game.material;

	// Can't plant footprints on fake materials (ladders, wading)
	if (mType != 'A' || 'B' || 'E' || 'F' || 'G' || 'H' || 'I' )
	{
		trace_t tr;
		UTIL_TraceLine((GetLocalOrigin()), (GetLocalOrigin() + Vector(0, 0, -256)),
		...
		QAngle vecAngles;
		VectorAngles(EyeDirection2D(), vecAngles);
		Vector right;
		GetVectors(NULL, &right, NULL);
		Vector footprint = tr.endpos;
		if (m_IsFootprintOnLeft) footprint -= right * 16;
		else footprint += right * 16;

		CEffectData	data;
		data.m_flRadius = 8;
		data.m_vNormal = tr.plane.normal;
		data.m_vOrigin = footprint;
		data.m_vAngles = GetAbsAngles();

		if (m_IsFootprintOnLeft)
		{
			DispatchEffect("footprint_left", data);
		}
		else
		{
			DispatchEffect("footprint_right", data);
		}
	}
...
#ifdef DARKINTERVAL
static ConCommand create_musclecar("create_musclecar", CC_CH_CreateJalopy, "", FCVAR_NONE);
#else
...
#ifdef DARKINTERVAL
static ConCommand create_buggy("create_buggy", CC_CH_CreateJeep, "", FCVAR_NONE);
#else
...
#ifdef DARKINTERVAL
static ConCommand create_airboat( "create_airboat", CC_CH_CreateAirboat, "", FCVAR_NONE);
#else
...
{
	// Cheat to create a Combine APC in front of the player
	Vector vecForward;
	AngleVectors(pPlayer->EyeAngles(), &vecForward);
	...
	if (pAPC)
	{
		...
		pAPC->KeyValue("model", "models/combine_apc_driveable.mdl");
		pAPC->KeyValue("solid", "6");
		pAPC->KeyValue("targetname", "my_apc");
		pAPC->KeyValue("vehiclescript", "scripts/vehicles/_driveable_apc.txt");
		DispatchSpawn(pAPC);
		pAPC->Activate();
...
static void CreateAPCNPC(CBasePlayer *pPlayer)
{
	// Cheat to create a Combine APC in front of the player
	Vector vecForward;
	AngleVectors(pPlayer->EyeAngles(), &vecForward);
	CBaseEntity *pAPC = (CBaseEntity *)CreateEntityByName("prop_vehicle_apc");
	CBaseEntity *pAPCdriver = (CBaseEntity *)CreateEntityByName("npc_apcdriver");
	if (pAPC && pAPCdriver)
	{
		Vector vecOrigin = pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0, 0, 64);
		QAngle vecAngles(0, pPlayer->GetAbsAngles().y - 90, 0);
		...
		pAPC->KeyValue("solid", "6");
		pAPC->KeyValue("targetname", "spawned_apc");
		pAPC->KeyValue("vehiclescript", "scripts/vehicles/apc_npc.txt");
		DispatchSpawn(pAPC);
		pAPC->Activate();
		pAPC->Teleport(&vecOrigin, &vecAngles, NULL);

		pAPCdriver->SetAbsOrigin(vecOrigin);
		pAPCdriver->SetAbsAngles(vecAngles);
		pAPCdriver->KeyValue("vehicle", "spawned_apc");
		pAPCdriver->KeyValue("targetname", "spawned_apc_driver");
		DispatchSpawn(pAPCdriver);
		pAPCdriver->Activate();
	}
}
...
static ConCommand create_apc("create_apc", CC_CH_CreateAPC, "", FCVAR_NONE);

void CC_CH_CreateAPCNPC(void)
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
	if (!pPlayer)
		return;
	CreateAPCNPC(pPlayer);
}

static ConCommand create_apc_npc("create_apc_npc", CC_CH_CreateAPCNPC, "", FCVAR_NONE);

static void CreateDigger(CBasePlayer *pPlayer)
{
	// Cheat to create a digger in front of the player
	Vector vecForward;
	AngleVectors(pPlayer->EyeAngles(), &vecForward);
	CBaseEntity *pDigger = (CBaseEntity *)CreateEntityByName("prop_vehicle_digger");
	if (pDigger)
	{
		Vector vecOrigin = pPlayer->GetAbsOrigin() + vecForward * 256 + Vector(0, 0, 64);
		QAngle vecAngles(0, pPlayer->GetAbsAngles().y - 90, 0);
		pDigger->SetAbsOrigin(vecOrigin);
		pDigger->SetAbsAngles(vecAngles);
		pDigger->KeyValue("model", "models/_Tech/Digger.mdl");
		pDigger->KeyValue("solid", "6");
		pDigger->KeyValue("targetname", "my_digger");
		pDigger->KeyValue("vehiclescript", "scripts/vehicles/_driveable_digger.txt");
		DispatchSpawn(pDigger);
		pDigger->Activate();
		pDigger->Teleport(&vecOrigin, &vecAngles, NULL);
	}
}

void CC_CH_CreateDigger(void)
{
	CBasePlayer *pPlayer = UTIL_GetCommandClient();
	if (!pPlayer)
		return;
	CreateDigger(pPlayer);
}

static ConCommand create_diggerc("create_digger", CC_CH_CreateDigger, "", FCVAR_NONE);
#endif // DARKINTERVAL
//=========================================================
//=========================================================
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
	...
	//	GiveNamedItem( "weapon_plasma");
	//	GiveAmmo(15,	"flaregun");
		GiveAmmo(150,	"TauRound");
		GiveAmmo(255,	"OICW");
		GiveAmmo(20,	"OICW_Grenade");
#endif
		if ( GetHealth() < 100 )
		{
	...
	case 107:
		{
#ifdef DARKINTERVAL
			engine->ClientCommand(edict(), "mat_crosshair");
#else
			trace_t tr;
			...
			Vector forward;
			EyeVectors( &forward );
			Vector end = start + forward * 33000;
...
bool CBasePlayer::ClientCommand( const CCommand &args )
{
	...
	else if (stricmp(cmd, "keypad_codematch"))
	{
		...
		{
			if (FClassnameIs(pEntity, "logic_keypad"))
			{
				edict_t *pFind;
				pFind = pEntity->edict();

				CBaseEntity *pEnt = CBaseEntity::Instance(pFind);
				CLogicKeypad *pKeypadSettings = (CLogicKeypad *)pEnt;
...
bool CBasePlayer::BumpWeapon( CBaseCombatWeapon *pWeapon )
{
	...
	else 
	{
		...
		////
			...
			//	ConColorMsg(Color(220, 210, 10), "Press %i to select %s\n", pWeapon->GetSlot() + 1, pWeapon->GetClassname());
			}
...
void CBasePlayer::UpdateClientData( void )
{
	...
	if (IsInAVehicle() && GetVehicleEntity()->m_takedamage != DAMAGE_NO)
	{
		m_iVehicleIntegrity = MAX(GetVehicleEntity()->m_iHealth, 0);
	}
	else
	{
		m_iVehicleIntegrity = -1;
	}

	if (m_iVehicleIntegrity != m_iClientVehicleIntegrity)
	{
		m_iClientVehicleIntegrity = m_iVehicleIntegrity;

		// send "battery" update message
		if (usermessages->LookupUserMessage("VehicleIntegrity") != -1)
		{
			UserMessageBegin(user, "VehicleIntegrity");
			WRITE_SHORT((int)m_iVehicleIntegrity);
			MessageEnd();
		}
	}
...
#if 0 // BYE BYE!!
	// Update Flashlight
	if ((m_flFlashLightTime) && (m_flFlashLightTime <= CURTIME))
	{
		if (FlashlightIsOn())
		{
			if (m_iFlashBattery)
			{
				m_flFlashLightTime = FLASH_DRAIN_TIME + CURTIME;
				m_iFlashBattery--;
		...
		else
		{
			...
			{
				m_flFlashLightTime = FLASH_CHARGE_TIME + CURTIME;
				m_iFlashBattery++;
			}
			else
	...
	// update the client with our paralyze state
	m_Local.m_bParalyzed = (m_bitsDamageType & DMG_PARALYZE)
		&& (m_nParalysisLasting > m_nParalysisGone)
		&& (m_iHealth < 100);
...
{
#ifdef DARKINTERVAL // very old prototype... not used anywhere
	if (pWeapon->NameMatches("weapon_broken"))
		return false;
#endif
	return true;
}
...
#ifdef DARKINTERVAL
	void InputStripWeaponsAndSuitAll(inputdata_t &data); // DI NEW - to strip worker suit too
	void InputStripSuitAll(inputdata_t &data); // DI NEW - to strip worker suit too but NOT weapons (for Eli's cave)
	void StripWeapons(inputdata_t &data, bool stripSuit, bool stripWorker = false, bool stripWeapons = true);
#else
...
#ifdef DARKINTERVAL
	DEFINE_INPUTFUNC( FIELD_VOID, "StripWeaponsAndSuitAll", InputStripWeaponsAndSuitAll), // DI NEW - to strip worker suit too
	DEFINE_INPUTFUNC( FIELD_VOID, "StripSuitAll", InputStripSuitAll), // DI NEW - to strip worker suit too but NOT weapons (for Eli's cave)
#endif
END_DATADESC()
#ifdef DARKINTERVAL
...
#endif // DARKINTERVAL
#ifdef DARKINTERVAL
void CStripWeapons::InputStripWeaponsAndSuitAll(inputdata_t &data) // DI NEW - to strip worker suit too
{
	StripWeapons(data, true, true, true);
}

void CStripWeapons::InputStripSuitAll(inputdata_t &data) // DI NEW - to strip worker suit too but NOT weapons (for Eli's cave)
{
	StripWeapons(data, true, true, false);
}

void CStripWeapons::StripWeapons(inputdata_t &data, bool stripSuit, bool stripWorker, bool stripWeapons)
#else
...
#endif
{
	...
	if ( pPlayer )
	{
		pPlayer->RemoveAllItems( stripSuit, stripWeapons );
	}
#ifdef DARKINTERVAL
	if (stripWorker)
	{
		pPlayer->m_Local.m_bSuitlessHUDVisible = false; // DI NEW - to strip worker suit too
	}
#endif
}
...
void CRevertSaved::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	if ( pPlayer )
	{
		// Setting this flag so we can't move or save a game.
...
#ifdef HL1_DLL
	SetNextThink( CURTIME + MessageTime() );
...
int CBasePlayer::GetFOV( void )
{
	...
	float deltaTime = (float)( CURTIME - m_flFOVTime ) / m_Local.m_flFOVRate;
...
int CBasePlayer::GetFOVForNetworking( void )
{
	...
	if ( CURTIME - m_flFOVTime < m_Local.m_flFOVRate )
	{
...
#ifdef DARKINTERVAL // self-explanatory: the long jump module
void CBasePlayer::EquipLongJump(bool bPlayEffects)
{
	m_Local.m_bWearingLongJump = true;
	...
	SetSuitUpdate("!HEVVOXNEW_LONGJUMP0", false, SUIT_NEXT_IN_30SEC);
}

void CBasePlayer::RemoveLongJump(void)
{
	m_Local.m_bWearingLongJump = false;
}
#endif
//-----------------------------------------------------------------------------
// Purpose: 
...
#ifdef DARKINTERVAL
// DI - patch back in the 'quti' command
static void QutiCallback(CBasePlayer *pPlayer)
{
	// Cheat to create a digger in front of the player. 
	Vector vecForward;
	AngleVectors(pPlayer->EyeAngles(), &vecForward);
