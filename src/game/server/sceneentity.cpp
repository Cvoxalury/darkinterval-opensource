...
#endif
#include "utlbuffer.h"
#ifndef DARKINTERVAL // DI ditched Episodic Alyx (for now) in favour of HL2 Alyx
#ifdef HL2_EPISODIC
...
#ifdef DARKINTERVAL
ConVar scene_debug("scene_debug", "0");
#endif
...
// Think every 50 msec
#define SCENE_THINK_INTERVAL 0.001 // FIXME: make scene's think in concept with their npc's	
...
#ifdef DARKINTERVAL
#define SF_CHOREOSCENE_PLAYONCE 1<<16 // easy way to delete the scene upon ending without map I/O
#endif
//===========================================================================================================
// SCENE LIST MANAGER
//===========================================================================================================
...
void LocalScene_Printf( const char *pFormat, ... )
{
	...
	Scene_Printf( "%s", msg );
	ADD_DEBUG_HISTORY( HISTORY_SCENE_PRINT, UTIL_VarArgs( "(%0.2f) %s", CURTIME, msg ) );
}
#endif
...
public:
	...
	{
		// no compilation at run time, only fetches
		Assert( 0 );
		return -1;
	}
...
void CSceneEntity::DispatchStartSpeak( CChoreoScene *scene, CBaseFlex *actor, CChoreoEvent *event, soundlevel_t iSoundlevel )
{
	...
	if ( actor )
	{
		...
		float soundtime = CURTIME - time_in_past;
...
#ifdef DARKINTERVAL	// DI NEW: we use player as actor for radio chatter. 
		// Because player can't be tested as the pActor is below, 
		// always make the scene that uses the player, wait for 
		// any active scene with player, to finish.
		else if (actor->IsPlayer())
		{
			CBasePlayer *player = ToBasePlayer(actor);
			if (player && player->GetExpresser())
			{
			//	Msg("note speaking for player\n");
				player->GetExpresser()->NoteSpeaking(flDuration, GetPostSpeakDelay());
			}
		}
#endif
		EmitSound_t es;
		...
		for ( int i = 0; i < c; ++i )
		{
			...
			speechListSounds[ speechListIndex ].time = CURTIME;
...
#ifdef DARKINTERVAL
					gender_t gender = gender_override.GetInt() ? (gender_t)gender_override.GetInt() : soundemitterbase->GetActorGender( pszActorModel );
#else
...
{
#ifdef DARKINTERVAL
	if (scene_debug.GetBool() && actor ) Msg("Scene entity %s (%s) : DispatchStartPermitResponses (length: %.1f) for actor %s\n", 
		GetDebugName(), scene->GetFilename(), event->GetDuration(), actor->GetDebugName());
#endif
	actor->SetPermitResponse( CURTIME + event->GetDuration() );
}
...
{
#ifdef DARKINTERVAL
	if(scene_debug.GetBool() && actor) Msg("Scene entity %s (%s) : DispatchEndPermitResponse for actor %s\n", 
		GetDebugName(), scene->GetFilename(), actor->GetDebugName());
#endif
...
bool CSceneEntity::CheckActors()
{
	...
	for ( i = 0 ; i < m_pScene->GetNumActors(); i++ )
	{
		...
		if ( !pTestActor->MyCombatCharacterPointer()->IsAlive() 
			...
			&& !pTestActor->IsPlayer() 
#endif
			) // DI change - allow dead player; we serve some taunts and things like that with player as the actor.
			return false;

		if ( m_BusyActor == SCENE_BUSYACTOR_WAIT )
		{
#ifdef DARKINTERVAL
			CBasePlayer *player = ToBasePlayer(pTestActor);

			if( player && player->GetExpresser() && player->GetExpresser()->IsSpeaking())
			{
				// DI NEW: we use player as actor for radio chatter. 
				// Because player can't be tested as the pActor is below, 
				// always make the scene that uses the player, wait for 
				// any active scene with player, to finish.
...
#ifdef DARKINTERVAL
	if (HasSpawnFlags(SF_CHOREOSCENE_PLAYONCE))
	{
		UTIL_Remove(this);
	}
#endif
...
void CSceneManager::Think()
{
	...
	SetNextThink( CURTIME + SCENE_THINK_INTERVAL );
	float frameTime = ( CURTIME - GetLastThink() );
...
void CSceneManager::OnClientActive( CBasePlayer *player )
{
	...
	for ( int i = 0; i < c; i++ )
	{
		...
		es.m_flSoundTime = CURTIME - sound->time_in_past;
...
#ifdef DARKINTERVAL // the in-game compiler doesn't work right now
...
/*
bool CreateSceneImageFileFromConsole(void)
{
	...
	bool bSuccess = g_pSceneImage->CreateSceneImageFile(targetBuffer, modDir, bLittleEndian, false, &statusHelper);
	if (bSuccess)
	{
		bSuccess = scriptlib->WriteBufferToFile(pFilename, targetBuffer, WRITE_TO_DISK_ALWAYS);
	}

	return bSuccess;
}
CON_COMMAND(scene_rebuild_image_file, "Rebuilds scene.image without needing Faceposer.")
{
	if (CreateSceneImageFileFromConsole()) Msg("Finished rebuilding local scene.image\n");
}
*/
#endif