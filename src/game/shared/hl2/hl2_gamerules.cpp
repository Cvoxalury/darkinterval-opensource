...
ConVar  physcannon_mega_enabled( "physcannon_mega_enabled", "0", FCVAR_NOTIFY | FCVAR_REPLICATED );
...
ConVar	sk_dmg_take_scale2( "sk_dmg_take_scale2", "1.20", FCVAR_REPLICATED );
ConVar	sk_dmg_take_scale3( "sk_dmg_take_scale3", "2.00", FCVAR_REPLICATED );
#else
...
#endif
#ifdef DARKINTERVAL
ConVar	sk_plr_dmg_flaregun("sk_plr_dmg_flaregun", "0", FCVAR_REPLICATED);
ConVar	sk_npc_dmg_flaregun("sk_npc_dmg_flaregun", "0", FCVAR_REPLICATED);
ConVar	sk_max_flaregun_ammo("sk_max_flaregun_ammo", "0", FCVAR_REPLICATED);

ConVar  sk_plr_dmg_tau("sk_plr_dmg_tau", "15", FCVAR_REPLICATED);
ConVar	sk_max_tau_ammo("sk_max_tau_ammo", "300", FCVAR_REPLICATED);
...
ConVar	sk_plr_dmg_oicw_alt("sk_plr_dmg_oicw_alt", "0", FCVAR_REPLICATED);
ConVar	sk_npc_dmg_oicw_alt("sk_npc_dmg_oicw_alt", "0", FCVAR_REPLICATED);
ConVar	sk_max_oicw_alt("sk_max_oicw_alt", "20", FCVAR_REPLICATED);

ConVar	sk_gunship_cannon_plr_dmg("sk_gunship_cannon_plr_dmg", "3", FCVAR_REPLICATED);
ConVar	sk_gunship_cannon_npc_dmg("sk_gunship_cannon_npc_dmg", "40", FCVAR_REPLICATED);

ConVar	sk_apc_gun_plr_dmg("sk_apc_gun_plr_dmg", "15", FCVAR_REPLICATED);
ConVar	sk_apc_gun_npc_dmg("sk_apc_gun_npc_dmg", "5", FCVAR_REPLICATED);

ConVar	sk_plr_dmg_slam("sk_plr_dmg_slam", "100", FCVAR_REPLICATED);
ConVar	sk_npc_dmg_slam("sk_npc_dmg_slam", "100", FCVAR_REPLICATED);
...
//		CBaseCombatCharacter::SetDefaultRelationship(CLASS_COMBINE_HUNTER,		CLASS_MILITARY,			D_NU, 0);
	...
	//	CBaseCombatCharacter::SetDefaultRelationship(CLASS_STALKER,			CLASS_MILITARY,			D_NU, 0);
		...
		CBaseCombatCharacter::SetDefaultRelationship(CLASS_VORTIGAUNT,		CLASS_BARNACLE,			D_NU, 0);
	...
	bool CHalfLife2::NPC_ShouldDropHealth( CBasePlayer *pRecipient )
	{
		...
		if ( m_flLastHealthDropTime > CURTIME )
			return false;
	...
	{
		// Can only do this every so often
		if ( m_flLastGrenadeDropTime > CURTIME )
			return false;
	...
	{
		m_flLastHealthDropTime = CURTIME + sk_plr_health_drop_time.GetFloat();
	}
	...
	{
		m_flLastGrenadeDropTime = CURTIME + sk_plr_grenade_drop_time.GetFloat();
	}
...
bool CHalfLife2::ShouldCollide( int collisionGroup0, int collisionGroup1 )
{
	...
	if (collisionGroup0 == HL2COLLISION_GROUP_HOUNDEYE && collisionGroup1 == HL2COLLISION_GROUP_HOUNDEYE )
		return false;
#ifdef DARKINTERVAL
	if (collisionGroup0 == DI_COLLISION_GROUP_HYDRA && collisionGroup1 == DI_COLLISION_GROUP_HYDRA)
		return false;

	if (collisionGroup0 == DI_COLLISION_GROUP_STICKYPROJECTILE && collisionGroup1 == DI_COLLISION_GROUP_STICKYPROJECTILE)
		return false;

	if (collisionGroup1 == DI_COLLISION_GROUP_STICKYPROJECTILE)
	{
		if (collisionGroup0 == COLLISION_GROUP_PLAYER)
			return false;

		if (collisionGroup0 == DI_COLLISION_GROUP_HYDRA)
			return false;
	}

	if (collisionGroup1 == DI_COLLISION_GROUP_HYDRA)
	{
		if (collisionGroup0 == COLLISION_GROUP_PLAYER)
			return false;
	}
	...
	if (collisionGroup1 == DI_COLLISION_GROUP_PLAYER_BARRIER)
	{
	...
	if (collisionGroup0 == HL2COLLISION_GROUP_HOMING_MISSILE && collisionGroup1 == HL2COLLISION_GROUP_HOMING_MISSILE )
		return false;
#ifdef DARKINTERVAL
	if (collisionGroup1 == HL2COLLISION_GROUP_HOMING_MISSILE)
	{
		if (collisionGroup0 == COLLISION_GROUP_DEBRIS)
			return false;
	...
	if ( collisionGroup0 == HL2COLLISION_GROUP_GUNSHIP && collisionGroup1 == HL2COLLISION_GROUP_GUNSHIP )
		return false;
#ifdef DARKINTERVAL
	// gunships and other users of this group don't collide with players
	if (collisionGroup0 == HL2COLLISION_GROUP_GUNSHIP && collisionGroup1 == COLLISION_GROUP_PLAYER)
		return false;
#endif
	...
	if ( collisionGroup0 == COLLISION_GROUP_WEAPON && (collisionGroup1 >= HL2COLLISION_GROUP_FIRST_NPC && collisionGroup1 <= HL2COLLISION_GROUP_LAST_NPC ) )
		return false; // DI CHANGE: make it not so. Why should guns or crowbars fly through NPCs?..
#endif
...
bool CHalfLife2::ShouldUseRobustRadiusDamage(CBaseEntity *pEntity)
{
	...
	if( !sv_robust_explosions.GetBool() )
		return false;
#ifdef DARKINTERVAL
	if( !pEntity->IsNPC() && !pEntity->IsPlayer() ) // DI CHANGE - allow to be used against players
#else
	if (!pEntity->IsNPC())
#endif
	{
...
#ifdef DARKINTERVAL
	if( pNPC && pNPC->CapabilitiesGet() & bits_CAP_SIMPLE_RADIUS_DAMAGE )
#else
...
{
#ifndef DARKINTERVAL
	if (alyx_darkness_force.GetBool()) return true;
#endif
...
CAmmoDef *GetAmmoDef()
{
	...
	if ( !bInitted )
	{
		...
		def.AddAmmoType("Grenade",			DMG_BURN,		TRACER_NONE,			"sk_plr_dmg_grenade", "sk_npc_dmg_grenade", "sk_max_grenade", 0, 0);
#else
		def.AddAmmoType("AR2",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	sk_plr_dmg_ar2.GetInt(),			sk_npc_dmg_ar2.GetInt(),			sk_max_ar2.GetInt(),			BULLET_IMPULSE(200, 1225), 0, 4, 8, 1.5f ); // AR2
		def.AddAmmoType("AlyxGun",			DMG_BULLET,					TRACER_LINE,			5,									3,									150,							BULLET_IMPULSE(200, 1225), 0 ); // Alyxgun
		def.AddAmmoType("Pistol",			DMG_BULLET,					TRACER_LINE_AND_WHIZ,	sk_plr_dmg_pistol.GetInt(),			sk_npc_dmg_pistol.GetInt(),			sk_max_pistol.GetInt(),			BULLET_IMPULSE(200, 1225), 0, 4, 8, 0.5f ); // Pistol
		def.AddAmmoType("SMG1",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	sk_plr_dmg_smg1.GetInt(),			sk_npc_dmg_smg1.GetInt(),			sk_max_smg1.GetInt(),			BULLET_IMPULSE(200, 1225), 0, 4, 8, 0.9f); // SMG
		def.AddAmmoType("357",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	sk_plr_dmg_357.GetInt(),			sk_npc_dmg_357.GetInt(),			sk_max_357.GetInt(),			BULLET_IMPULSE(800, 5000), 0, 4, 8, 2.0f); // 357
		def.AddAmmoType("XBowBolt",			DMG_BULLET,					TRACER_LINE,			sk_plr_dmg_crossbow.GetInt(),		sk_npc_dmg_crossbow.GetInt(),		sk_max_crossbow.GetInt(),		BULLET_IMPULSE(800, 8000), 0, 4, 8, 2.0f); // XBow
		def.AddAmmoType("Buckshot",			DMG_BULLET | DMG_BUCKSHOT,  TRACER_LINE_AND_WHIZ,	sk_plr_dmg_buckshot.GetInt(),		sk_npc_dmg_buckshot.GetInt(),		sk_max_buckshot.GetInt(),		BULLET_IMPULSE(400, 1200), 0, 4, 10, 0.75f ); // Buckshot
		def.AddAmmoType("RPG_Round",		DMG_BURN,					TRACER_NONE,			sk_plr_dmg_rpg_round.GetInt(),		sk_npc_dmg_rpg_round.GetInt(),		sk_max_rpg_round.GetInt(),		0, 0 ); // RPG
		def.AddAmmoType("SMG1_Grenade",		DMG_BURN,					TRACER_NONE,			sk_plr_dmg_smg1_grenade.GetInt(),	sk_npc_dmg_smg1_grenade.GetInt(),	sk_max_smg1_grenade.GetInt(),	0, 0 ); // SMG Grenade
		def.AddAmmoType("SniperRound",		DMG_BULLET | DMG_SNIPER,	TRACER_NONE,			sk_plr_dmg_sniper_round.GetInt(),	sk_npc_dmg_sniper_round.GetInt(),	sk_max_sniper_round.GetInt(),	BULLET_IMPULSE(650, 6000), 0 ); // Sniper
		def.AddAmmoType("SniperPenetratedRound", DMG_BULLET | DMG_SNIPER, TRACER_NONE,			25,									100,								10,								BULLET_IMPULSE(150, 6000), 0 ); // Sniper, Penetrating
		def.AddAmmoType("Grenade",			DMG_BURN,					TRACER_NONE,			sk_plr_dmg_grenade.GetInt(),		sk_npc_dmg_grenade.GetInt(),		sk_max_grenade.GetInt(),		0, 0); // Grenade
		def.AddAmmoType("Thumper",			DMG_SONIC,					TRACER_NONE,			10,									10,									2,								0, 0 ); // Thumper
		def.AddAmmoType("Gravity",			DMG_CLUB,					TRACER_NONE,			0,									0,									8,								0, 0 ); // Gravity
		def.AddAmmoType("Battery",			DMG_CLUB,					TRACER_NONE,			NULL,								NULL,								NULL,							0, 0 ); // Battery
		def.AddAmmoType("GaussEnergy",		DMG_SHOCK,					TRACER_NONE,			25,									30,									30,								BULLET_IMPULSE(650, 8000), 0, 4, 8, 2.0f ); // hit like a 10kg weight at 400 in/s // Gauss
		def.AddAmmoType("CombineCannon",	DMG_BULLET,					TRACER_LINE_AND_WHIZ,	sk_gunship_cannon_plr_dmg.GetInt(), sk_gunship_cannon_npc_dmg.GetInt(), NULL, 1.5 * 750 * 12, 0); // hit like a 1.5kg weight at 750 ft/s // Combine Cannon
		def.AddAmmoType("AirboatGun",		DMG_AIRBOAT,				TRACER_LINE,			3,									3,									NULL,							BULLET_IMPULSE(10, 600), 0 ); // Airboat
		def.AddAmmoType("StriderMinigun",	DMG_BULLET,					TRACER_LINE,			5,									15,									15,								1.0 * 750 * 12, AMMO_FORCE_DROP_IF_CARRIED, 6, 12 ); // hit like a 1.0kg weight at 750 ft/s // Strider Minigun
		def.AddAmmoType("StriderMinigunDirect",	DMG_BULLET,				TRACER_LINE,			2,									2,									15,								1.0 * 750 * 12, AMMO_FORCE_DROP_IF_CARRIED ); // hit like a 1.0kg weight at 750 ft/s // Strider Minigun, direct
		def.AddAmmoType("HelicopterGun",	DMG_BULLET,					TRACER_LINE_AND_WHIZ,	3,									6,									500,							BULLET_IMPULSE(400, 1225), AMMO_FORCE_DROP_IF_CARRIED | AMMO_INTERPRET_PLRDAMAGE_AS_DAMAGE_TO_PLAYER, 6, 12 ); // Helicopter
		def.AddAmmoType("AR2AltFire",		DMG_DISSOLVE,				TRACER_NONE,			0,									0,									sk_max_ar2_altfire.GetInt(),	0, 0 ); // AR2 Altfire
		def.AddAmmoType("CombineHeavyCannon",	DMG_BULLET,				TRACER_LINE,			40,									40,									NULL,							10 * 750 * 12, AMMO_FORCE_DROP_IF_CARRIED ); // hit like a 10 kg weight at 750 ft/s // Combine Heavy Cannon

		// DARKINTERVAL-specific ammo
		def.AddAmmoType("APCTurret",		DMG_AIRBOAT,				TRACER_LINE_AND_WHIZ,			sk_apc_gun_plr_dmg.GetInt(),		sk_apc_gun_npc_dmg.GetInt(),		NULL,							BULLET_IMPULSE(650, 8000), 0, 4, 8, 2.0f); // hit like a 10kg weight at 400 in/s // APCTurret
		def.AddAmmoType("Flaregun",			DMG_BULLET,					TRACER_NONE,			sk_plr_dmg_flaregun.GetInt(),		sk_npc_dmg_flaregun.GetInt(),		sk_max_flaregun_ammo.GetInt(),	0, 0); // Flaregun
		def.AddAmmoType("TauRound",			DMG_BULLET,					TRACER_RAIL,			sk_plr_dmg_tau.GetInt(),			0,									sk_max_tau_ammo.GetInt(),		BULLET_IMPULSE(150, 1000), 0, 4, 8, 2.5f); // Tau Round
		def.AddAmmoType("MeleeTrace_Club",	DMG_CLUB,					TRACER_NONE,			1,									1,									1,								(5 * 100 * 12), 0);
		def.AddAmmoType("MeleeTrace_Slash", DMG_SLASH,					TRACER_NONE,			1,									1,									1,								8, 0);
		def.AddAmmoType("OICW",				DMG_BULLET,					TRACER_LINE_AND_WHIZ,	sk_plr_dmg_oicw.GetInt(),			sk_npc_dmg_oicw.GetInt(),			sk_max_oicw.GetInt(),			BULLET_IMPULSE(200, 1225), 0, 4, 8, 1.5f); // OICW
		def.AddAmmoType("OICW_Grenade",		DMG_BURN,					TRACER_LINE,			sk_plr_dmg_oicw_alt.GetInt(),		sk_npc_dmg_oicw_alt.GetInt(),		sk_max_oicw_alt.GetInt(),		0, 0, 4, 8, 4.0f); // OICW altfire
		def.AddAmmoType("SLAM",				DMG_BURN,					TRACER_NONE,			sk_plr_dmg_slam.GetInt(),			sk_npc_dmg_slam.GetInt(),			sk_max_slam.GetInt(), 0, 0);	// SLAM
#endif
	}
