...
// DARKINTERVAL NOTE: This isn't ifdef'd, because that'll be a lot of extra lines and
// defeat the purpose of code brevity. But, a lot of instances of "CURTIME"
// were replaced (over time, all should be replaced) with this macro. 
#define CURTIME					(gpGlobals->curtime)
...
#ifdef DARKINTERVAL
#define GLOWS_ENABLE
//#define PORTAL_COMPILE
//#define PORTAL_COMPILE_SIMULATORS_EMBED_GUID
#endif
...
#define MAX_WEAPON_POSITIONS	3	// max number of items within a slot // DI CHANGE: but only up to 4 in a stack
...
#ifdef DARKINTERVAL
#define HIDEHUD_ARCADE_HUD			( 1<<12 )	// the green health bar that replaced the normal hud when in Versus
...
#ifdef DARKINTERVAL // it is singleplayer for now
...
#ifdef DARKINTERVAL
#define MAX_FOV						150 // while it causes skybox issues, some players complain that it's not increased
#else
#define MAX_FOV						90
#endif
//===================================================================================================================
...
#ifdef DARKINTERVAL // energybeam isn't used anywhere, but we want freeze used in places
#define DMG_FREEZE			(1 << 10)	// cold damage
#else
...
#ifdef DARKINTERVAL
#define DMG_DISSOLVE_BURN	(1<<30)		// Immolator DMG

// TODO: keep this up to date so all the mod-specific flags don't overlap anything.
#define DMG_LASTGENERICFLAG	DMG_DISSOLVE_BURN
#else
...
enum
{
...
#endif
	EFL_DIRTY_SHADOWUPDATE =		(1<<5),			// Need shadow manager to update this entity's shadow.
...
#ifdef DARKINTERVAL
	EFL_HAS_PLAYER_CHILD =			(1<<8),			// One of the child entities is a player.
#else
	...
	EFL_NO_ROTORWASH_PUSH =			(1<<21),		// Not affected by rotorwash forces.
	...
	EFL_NO_DISSOLVE =				(1<<27),		// Disable dissolve mechanic on this entity.
#ifdef DARKINTERVAL
	// UNUSED =						(1<<28),		 
#else	
