...
#ifdef DARKINTERVAL // all this is for restoring the pure-txt, no-dat based closecaptions.
...
static void CC_LookupCaptionCRC(const CCommand &args)
{
	...
	CRC32_t crc = (CRC32_t)atoi(args[1]);
...
EmitSound_t::EmitSound_t( const CSoundParameters &src )
{
	...
	m_nSpecialDSP = 0;
	m_pOrigin = 0;
	m_flSoundTime = ( src.delay_msec == 0 ) ? 0.0f : CURTIME + ( (float)src.delay_msec / 1000.0f );
...
public:
	...
	void EmitSoundByHandle( IRecipientFilter& filter, int entindex, const EmitSound_t & ep, HSOUNDSCRIPTHANDLE& handle )
	{
		...
		if ( ent )
		{
			...
			gender = 
#ifdef DARKINTERVAL
				gender_override.GetInt() ? (gender_t)gender_override.GetInt() : 
#endif
				soundemitterbase->GetActorGender( actorModel );
		}
		...
		if ( !st && 
			params.delay_msec != 0 )
		{
			st = CURTIME + (float)params.delay_msec / 1000.f;
		}
		...
		if ( filterCopy.GetRecipientCount() > 0 )
		{
			...
			if ( pActor )
			{
				...
				gender_t gender = 
					...
					gender_override.GetInt() ? (gender_t)gender_override.GetInt() : 
#endif
					soundemitterbase->GetActorGender( pszActorModel );
